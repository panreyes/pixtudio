...
int file_writeSint16A(file *fp, int16_t *buffer, int n) {
    ...
    int i;
    for (i = 0; i < n; i++)
        if (!file_writeSint16(fp, (int16_t *)buffer++))
            break;
    return i;
#endif
}
...
int file_writeSint32A(file *fp, int32_t *buffer, int n) {
    ...
    int i;
    for (i = 0; i < n; i++)
        if (!file_writeSint32(fp, (int32_t *)buffer++))
            break;
    return i;
#endif
}
...
int file_readSint16A(file *fp, int16_t *buffer, int n) {
    ...
    int i;
    for (i = 0; i < n; i++)
        if (!file_readSint16(fp, (int16_t *)buffer++))
            break;
    return i;
#endif
}
...
int file_readSint32A(file *fp, int32_t *buffer, int n) {
    ...
    int i;
    for (i = 0; i < n; i++)
        if (!file_readSint32(fp, (int32_t *)buffer++))
            break;
    return i;
#endif
}
...
file *file_open(const char *filename, char *mode) {
    ...
    *p = '\0';
    ...
    /* Use file extension for search in a directory named as it (for example: FPG dir for .FPG
     * files) */
    if (strchr(name, '.')) {
