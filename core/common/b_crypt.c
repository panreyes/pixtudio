...
crypt_handle *crypt_create(int method, char *key) {
    ...
    switch (method) {
        case CRYPT_DES:
#ifdef USE_LIBDES
            if (!DES_key_sched((DES_cblock *)key, ch->ks[KEY0]))
                return ch;
#else
            if (!DES_key_sched((DES_cblock *)key, &ch->ks[KEY0]))
                return ch;
        ...
        case CRYPT_3DES:
            ...
            if (DES_key_sched((DES_cblock *)(8 + key), ch->ks[KEY1])) {
                free(ch);
                return NULL;
            }
            if (!DES_key_sched((DES_cblock *)(16 + key), ch->ks[KEY2]))
                return ch;
#else
            if (DES_key_sched((DES_cblock *)key, &ch->ks[KEY0])) {
                free(ch);
