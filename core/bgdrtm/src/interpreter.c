    ...
    while (first_instance) {
        if (debug_mode) {
            ...
            while (i) {
                ...
                /* If instance is KILLED or DEAD or return from some debug command, then execute it
                   again.
    ...
    while (!must_exit) {
        ...
        if (debug_DCB > 0) {
            if (debug_DCB > 2) {
                ...
                if (debug_DCB > 1)
                    BGDRTM_LOG("%*.*s[%4u] ", c, c, "", (ptr - r->code));
            } else if (debug_DCB > 1)
                BGDRTM_LOG("[%4u] ", (ptr - r->code));
        ...
        switch (*ptr) {
            ...
            case MN_PROC: {
                ...
                if (!proc) {
                    BGDRTM_LOGERROR("ERROR: Runtime error in %s(%d) - Unknown process\n",
                ...
                if (child_is_alive && (((status = LOCDWORD(r, STATUS)) & STATUS_WAITING_MASK) ||
            ...
            case MN_SYSCALL:
                p = sysproc_get(ptr[1]);
                if (!p) {
                    BGDRTM_LOGERROR("ERROR: Runtime error in %s(%d) - Unknown system function\n",
                            r->proc->name, LOCDWORD(r, PROCESS_ID));
                    exit(0);
                }
            ...
            case MN_SYSPROC:
                p = sysproc_get(ptr[1]);
                if (!p) {
                    BGDRTM_LOGERROR("ERROR: Runtime error in %s(%d) - Unknown system process\n",
                            r->proc->name, LOCDWORD(r, PROCESS_ID));
                    exit(0);
                }
                r->stack_ptr -= p->params;
            ...
            case MN_REMOTE | MN_FLOAT:
                ...
                if (!i) {
                    BGDRTM_LOGERROR("ERROR: Runtime error in %s(%d) - Process %d not active\n",
            ...
            case MN_FLOAT2INT:
                *(int32_t *)&(r->stack_ptr[-ptr[1] - 1]) =
                    (int32_t) * (float *)&(r->stack_ptr[-ptr[1] - 1]);
                ptr += 2;
                break;

            case MN_INT2FLOAT:
            case MN_INT2FLOAT | MN_UNSIGNED:
                *(float *)&(r->stack_ptr[-ptr[1] - 1]) =
                    (float)*(int32_t *)&(r->stack_ptr[-ptr[1] - 1]);
                ptr += 2;
                break;

            case MN_INT2FLOAT | MN_UNSIGNED | MN_WORD:
                *(float *)&(r->stack_ptr[-ptr[1] - 1]) =
                    (float)*(uint16_t *)&(r->stack_ptr[-ptr[1] - 1]);
                ptr += 2;
                break;

            case MN_INT2FLOAT | MN_UNSIGNED | MN_BYTE:
                *(float *)&(r->stack_ptr[-ptr[1] - 1]) =
                    (float)*(uint8_t *)&(r->stack_ptr[-ptr[1] - 1]);
                ptr += 2;
                break;

            case MN_INT2WORD:
            case MN_INT2WORD | MN_UNSIGNED:
                *(uint32_t *)&(r->stack_ptr[-ptr[1] - 1]) =
                    (int32_t)(uint16_t) * (int32_t *)&(r->stack_ptr[-ptr[1] - 1]);
                ptr += 2;
                break;
            case MN_INT2BYTE:
            case MN_INT2BYTE | MN_UNSIGNED:
                *(uint32_t *)&(r->stack_ptr[-ptr[1] - 1]) =
                    (int32_t)(uint8_t) * (int32_t *)&(r->stack_ptr[-ptr[1] - 1]);
                ptr += 2;
                break;
            ...
            case MN_DIV:
                if (r->stack_ptr[-1] == 0) {
                    BGDRTM_LOGERROR("ERROR: Runtime error in %s(%d) - Division by zero\n",
                            r->proc->name, LOCDWORD(r, PROCESS_ID));
                    exit(0);
                }
                r->stack_ptr[-2] /= r->stack_ptr[-1];
            ...
            case MN_STRACAT:
                n = r->stack_ptr[-1];
                strncat(*(char **)(&r->stack_ptr[-2]), string_get(n),
                        (ptr[1] - 1) - strlen(*(char **)(&r->stack_ptr[-2])));
            ...
            case MN_SWITCH | MN_STRING:
                if (r->switchval_string != 0)
                    string_discard(r->switchval_string);
            ...
            case MN_CASE:
                if (r->switchval == *--r->stack_ptr)
                    r->cased = 2;
                ptr++;
                break;

            case MN_CASE | MN_STRING:
                if (string_comp(r->switchval_string, *--r->stack_ptr) == 0)
                    r->cased = 2;
            ...
            case MN_CASE_R:
                r->stack_ptr -= 2;
                if (r->switchval >= r->stack_ptr[0] && r->switchval <= r->stack_ptr[1])
                    r->cased = 1;
                ptr++;
                break;
            ...
            case MN_TYPE: {
                ...
                if (!proct) {
                    BGDRTM_LOGERROR("ERROR: Runtime error in %s(%d) - Invalid type\n",
                            r->proc->name, LOCDWORD(r, PROCESS_ID));
                    exit(0);
                }
            ...
            case MN_FRAME:
                ...
                if (!(r->proc->flags & PROC_FUNCTION) && r->called_by &&
            ...
            default:
                BGDRTM_LOGERROR("ERROR: Runtime error in %s(%d) - Mnemonic 0x%02X not implemented\n",
                                r->proc->name, LOCDWORD(r, PROCESS_ID), *ptr);
                exit(0);
        }

        if (r->stack_ptr < r->stack) {
            BGDRTM_LOGERROR("ERROR: Runtime error in %s(%d) - Critical Stack Problem StackBase=%p "
                            "StackPTR=%p\n",
...
break_all:
    ...
    /* Hook */
    if (r && LOCDWORD(r, STATUS) != STATUS_KILLED && r->first_run)
        r->first_run = 0;
