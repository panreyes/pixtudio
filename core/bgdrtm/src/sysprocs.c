...
static void get_var_info(DLVARFIXUP *varfixup, DCB_VAR *basevar, int nvars, char *basedata) {
    ...
    for (;;) {
        if (token.name[0] == '.') {
            ...
            if (rvar.Type.BaseType[0] != TYPE_STRUCT)
                return; /* not an struct */
        ...
        if (token.name[0] == '[') {
            ...
            if (rvar.Type.BaseType[0] != TYPE_ARRAY)
                return; /* not an array */
            ...
            if (index < 0)
                return; /* Index less than zero */
            if (index >= rvar.Type.Count[0])
