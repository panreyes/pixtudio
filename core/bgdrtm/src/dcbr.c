...
void sysprocs_fixup(void) {
    ...
    while (proc->func) {
        ...
        for (n = 0; n < dcb.data.NSysProcsCodes; n++, s++) {
            if (proc->type == s->Type && proc->params == s->Params && s->Id == getid(proc->name) &&
                !strcmp((const char *)s->ParamTypes, proc->paramtypes)) {
                proc->code = s->Code;
                break;
            }
        }
...
DCB_VAR *read_and_arrange_varspace(file *fp, int count) {
    ...
    for (n = 0; n < count; n++) {
        ...
        for (n1 = 0; n1 < MAX_TYPECHUNKS; n1++)
            ARRANGE_DWORD(&vars[n].Type.Count[n1]);
...
int dcb_load_from(file *fp, const char *filename, int offset) {
    ...
    if (memcmp(dcb.data.Header, DCB_MAGIC, sizeof(DCB_MAGIC) - 1) != 0 ||
    ...
    for (n = 0; n < dcb.data.NProcs; n++) {
        ...
        if (dcb.proc[n].data.SPrivate) {
            procs[n].pridata = (int *)calloc(dcb.proc[n].data.SPrivate,
                                             sizeof(char)); /* El size ya esta calculado en bytes */
        ...
        if (dcb.proc[n].data.SPublic) {
            procs[n].pubdata = (int *)calloc(dcb.proc[n].data.SPublic,
                                             sizeof(char)); /* El size ya esta calculado en bytes */
        ...
        if (dcb.proc[n].data.SCode) {
            procs[n].code = (int *)calloc(dcb.proc[n].data.SCode,
                                          sizeof(char)); /* El size ya esta calculado en bytes */
            file_seek(fp, offset + dcb.proc[n].data.OCode, SEEK_SET);
            file_readUint32A(fp, (uint32_t *)procs[n].code,
