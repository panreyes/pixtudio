...
static char **string_ptr =
    NULL; /* Pointers to each string's text. Every string is allocated using strdup() or malloc().
             A pointer of a unused slot is 0.
             Exception: "fixed" strings are stored in a separate memory block and should not be
             freed */
static uint32_t *string_uct =
    NULL; /* Usage count for each string. An unused slot has a count of 0 */

static uint32_t *string_bmp =
    NULL; /* Bitmap for speed up string creation, and reused freed slots */

static int string_allocated =
    0; /* How many string slots are available in the ptr, uct and dontfree arrays */

static int string_bmp_start = 0; /* Offset of assignable string for reused (32bits each one) */

static int string_last_id =
    1; /* How many strings slots are used. This is only the bigger id in use + 1.
...
static void string_alloc(int count) {
    ...
    if (!string_ptr || !string_uct || !string_bmp) {
        PXTRTM_LOGERROR("ERROR: Runtime error - string_alloc: out of memory\n");
        exit(0);
    }
...
void string_dump(void (*wlog)(const char *fmt, ...)) {
    ...
    if (wlog)
        ...
        PXTRTM_LOG("[STRING] ---- Dumping MaxID=%d strings ----\n", string_allocated);
    ...
    if (wlog)
        ...
        PXTRTM_LOG("[STRING] ---- Dumping Used=%d End ----\n", used);
}
...
void string_load(void *fp, int ostroffs, int ostrdata, int nstrings, int totalsize) {
    ...
    if (string_last_id + nstrings > string_allocated)
        string_alloc(((string_last_id + nstrings - string_allocated) / BLOCK_INCR + 1) *
                     BLOCK_INCR);
...
void string_discard(int code) {
    if (code < 0 || code > string_allocated || !string_ptr[code])
        return;
