...
void instance_add_to_list_by_id(INSTANCE *r, uint32_t id) {
    if (!hashed_by_id)
        hashed_by_id = calloc(HASH_SIZE, sizeof(INSTANCE *));
...
void instance_add_to_list_by_instance(INSTANCE *r) {
    ...
    if (!hashed_by_instance)
        hashed_by_instance = calloc(HASH_SIZE, sizeof(INSTANCE *));

    r->prev_by_instance = NULL;
    r->next_by_instance = hashed_by_instance[hash];
    if (r->next_by_instance)
        r->next_by_instance->prev_by_instance = r;
...
void instance_remove_from_list_by_instance(INSTANCE *r) {
    ...
    if (r->prev_by_instance)
        r->prev_by_instance->next_by_instance = r->next_by_instance;
    if (r->next_by_instance)
        r->next_by_instance->prev_by_instance = r->prev_by_instance;

    if (hashed_by_instance[hash] == r)
        hashed_by_instance[hash] = r->next_by_instance;
}
...
void instance_add_to_list_by_type(INSTANCE *r, uint32_t type) {
    ...
    if (!hashed_by_type)
        hashed_by_type = calloc(HASH_SIZE, sizeof(INSTANCE *));

    r->prev_by_type = NULL;
    r->next_by_type = hashed_by_type[hash];
    if (r->next_by_type)
        r->next_by_type->prev_by_type = r;
...
void instance_remove_from_list_by_type(INSTANCE *r, uint32_t type) {
    ...
    if (r->prev_by_type)
        r->prev_by_type->next_by_type = r->next_by_type;
    if (r->next_by_type)
        r->next_by_type->prev_by_type = r->prev_by_type;

    if (hashed_by_type[hash] == r)
        hashed_by_type[hash] = r->next_by_type;
}
...
void instance_add_to_list_by_priority(INSTANCE *r, int32_t priority) {
    ...
    if (priority < INSTANCE_MIN_PRIORITY)
        priority = LOCINT32(r, PRIORITY) = INSTANCE_MIN_PRIORITY;
    if (priority > INSTANCE_MAX_PRIORITY)
        priority = LOCINT32(r, PRIORITY) = INSTANCE_MAX_PRIORITY;

    hash = HASH_PRIORITY(priority);

    if (!hashed_by_priority)
        hashed_by_priority = calloc(HASH_SIZE, sizeof(INSTANCE *));

    r->prev_by_priority = NULL;
    r->next_by_priority = hashed_by_priority[hash];
    if (r->next_by_priority)
        r->next_by_priority->prev_by_priority = r;
    hashed_by_priority[hash]                  = r;
    r->last_priority                          = priority;

    if (priority < instance_min_actual_prio)
        instance_min_actual_prio = priority;
    if (priority > instance_max_actual_prio)
        instance_max_actual_prio = priority;
}
...
void instance_remove_from_list_by_priority(INSTANCE *r) {
    ...
    if (iterator_by_priority == r)
        instance_next_by_priority();
    ...
    if (hashed_by_priority[hash] == r)
        hashed_by_priority[hash] = r->next_by_priority;

    if (!hashed_by_priority[hash]) {
        if (r->last_priority == instance_min_actual_prio) {
            while (instance_min_actual_prio < instance_max_actual_prio &&
                   !hashed_by_priority[instance_min_actual_prio + INSTANCE_NORMALIZE_PRIORITY])
                instance_min_actual_prio++;
        }

        if (r->last_priority == instance_max_actual_prio) {
            while (instance_max_actual_prio > instance_min_actual_prio &&
                   !hashed_by_priority[instance_max_actual_prio + INSTANCE_NORMALIZE_PRIORITY])
...
INSTANCE *instance_duplicate(INSTANCE *father) {
    ...
    if (father->private_size > 0)
        memcpy(r->pridata, father->pridata, r->private_size);
    if (father->public_size > 0)
        memcpy(r->pubdata, father->pubdata, r->public_size);
    if (local_size > 0)
    ...
    for (n = 0; n < r->proc->string_count; n++)
        string_use(PRIDWORD(r, r->proc->strings[n])); /* Strings privadas */
    for (n = 0; n < r->proc->pubstring_count; n++)
        string_use(PUBDWORD(r, r->proc->pubstrings[n])); /* Strings publicas */
    for (n = 0; n < local_strings; n++)
        string_use(LOCDWORD(r, localstr[n])); /* Strings locales */

    r->prev = NULL;
    r->next = first_instance;
    if (first_instance)
        first_instance->prev = r;
    first_instance           = r;
...
INSTANCE *instance_new(PROCDEF *proc, INSTANCE *father) {
    ...
    for (n = 0; n < proc->string_count; n++)
        string_use(PRIDWORD(r, proc->strings[n])); /* Strings privadas */
    for (n = 0; n < proc->pubstring_count; n++)
        string_use(PUBDWORD(r, proc->pubstrings[n])); /* Strings publicas */
    for (n = 0; n < local_strings; n++)
        string_use(LOCDWORD(r, localstr[n])); /* Strings locales */
...
void instance_destroy(INSTANCE *r) {
    ...
    for (n = 0; n < r->proc->string_count; n++)
        string_discard(PRIDWORD(r, r->proc->strings[n])); /* Strings privadas */
    for (n = 0; n < r->proc->pubstring_count; n++)
        string_discard(PUBDWORD(r, r->proc->pubstrings[n])); /* Strings publicas */
    for (n = 0; n < local_strings; n++)
        string_discard(LOCDWORD(r, localstr[n])); /* Strings locales */

    /* Actualiza árbol de jerarquias */

    bigbro = instance_get(LOCDWORD(r, BIGBRO)); /* Tengo hermano mayor? */
    if (bigbro)
        LOCDWORD(bigbro, SMALLBRO) =
            LOCDWORD(r, SMALLBRO); /* El hermano menor de mi hermano mayor es mi hermano menor */

    smallbro = instance_get(LOCDWORD(r, SMALLBRO)); /* Tengo hermano menor? */
    if (smallbro)
        LOCDWORD(smallbro, BIGBRO) =
            LOCDWORD(r, BIGBRO); /* El hermano mayor de mi hermano menor es mi hermano mayor */
...
INSTANCE *instance_next_by_priority() {
    ...
    if (!iterator_by_priority) {
        ...
        while (--iterator_pos >= (instance_min_actual_prio + INSTANCE_NORMALIZE_PRIORITY) &&
               !(iterator_by_priority = hashed_by_priority[iterator_pos]))
            ;
    }
...
INSTANCE *instance_get_by_type(uint32_t type, INSTANCE **context) {
    ...
    if (!context || !hashed_by_type || !type /* || type >= FIRST_INSTANCE_ID */)
        return NULL;
