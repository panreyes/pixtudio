...
__DIR_ST *dir_open(const char *path) {
    ...
    *fptr = 0;
    ...
    /* Convert '*.*' to '*' */
    if (fptr > hDir->pattern + 2 && fptr[-1] == '*' && fptr[-2] == '.' && fptr[-3] == '*')
        fptr[-2] = 0;
...
__DIR_FILEINFO_ST *dir_read(__DIR_ST *hDir) {
    ...
    hDir->info.attributes =
        ((hDir->data.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) ? DIR_FI_ATTR_DIRECTORY : 0) |
    ...
    if (!FindNextFile(hDir->handle, &hDir->data))
        hDir->eod = 1;
#else
    struct stat s;

    if (hDir->currFile == hDir->globd.gl_pathc)
        return NULL;
