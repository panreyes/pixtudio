...
int line_count =
    0; /* Se pone a 0, ya que lo incremente con cada \n, y hasta no obtener un \n no se procesa la
          linea (Splinter) */
int current_file = 0;
...
int load_file(char *filename) {
    ...
    for (n = 0; n < n_files; n++)
        if (!strcmp(files[n], filename))
            break;

    if (n >= n_files) {
        if (n_files == MAX_SOURCES)
            compile_error(MSG_TOO_MANY_FILES);
        strcpy(files[n_files], filename);
        if (!fp)
            compile_error(MSG_FILE_NOT_FOUND, filename);
        size                 = file_size(fp);
        source_data[n_files] = (char *)calloc(size + 1, sizeof(char));
        if (!source_data[n_files])
            compile_error(MSG_FILE_TOO_BIG, filename);
        if (size == 0)
            compile_error(MSG_FILE_EMPTY, filename);
        if (!file_read(fp, source_data[n_files], size))
            compile_error(MSG_READ_ERROR, filename);
...
void add_simple_define(char *macro, char *text) {
    ...
    if (find_define(code) != -1)
        compile_error(MSG_MACRO_ERROR, identifier_name(code));
...
void preprocessor_jumpto(int id, int id2) {
            ...
            if (*source_ptr == '#') {
                ...
                if (token.type == IDENTIFIER) {
                    if (token.code == id_endif) {
                        ...
                        if (prepro_sp < 0 || (stck_code != id_else && stck_code != id_if &&
                                              stck_code != id_ifdef && stck_code != id_ifndef))
                            compile_error("#endif without #if");
                    }
                    if (token.code == id_else) {
                        int stck_code = prepro_stack[--prepro_sp];

                        if (prepro_sp < 0 || (stck_code != id_if && stck_code != id_ifdef &&
                                              stck_code != id_ifndef)) {
                    ...
                    } else if (token.code == id_ifdef || token.code == id_ifndef ||
                               token.code == id_if) {
                        prepro_stack[prepro_sp++] = token.code;
                    }
    ...
    if (token.code != id && (id2 && token.code != id2))
        compile_error("unbalanced #if/#else/#endif");

    if (depth > 0)
        compile_error("unterminate #if");
        ...
        for (count = 0; count < def->param_count; count++) {
            ...
            while (*source_ptr && (depth > 0 || (*source_ptr != ')' && *source_ptr != ','))) {
                if (*source_ptr == '"' || *source_ptr == '\'') {
                    begin = source_ptr++;
                    while (*source_ptr && *source_ptr != *begin)
                        source_ptr++;
                    if (!*source_ptr)
                        compile_error(MSG_EXPECTED, "\"");
                    source_ptr++;
...
void preprocessor() {
    ...
    if (token.code == id_define) {
        ...
        if (find_define(token.code) != -1)
            compile_error(MSG_MACRO_ERROR, identifier_name(token.code));
    ...
    if (token.code == id_ifdef || token.code == id_ifndef) {
        ...
        if (*source_ptr && *source_ptr != '\n') {
            if (ifdef)
                compile_warning(0, "extra tokens at end of #ifdef directive");
            else
                compile_warning(0, "extra tokens at end of #ifndef directive");
            SKIP_ALL_UNTIL_LF_AND_COUNT_LINES;
        }
    ...
    if (token.code == id_if) {
        ...
        *ptr = '\0';
        ...
        /*
        printf ("exp: asignation: [%d] call: [%d] lvalue: [%d] constant: [%d] value: [%d] lvalue:
        [%f] type: [%d]\n",
                      res.asignation,
                      res.call,
                      res.lvalue,
    ...
    if (token.code == id_else) {
        ...
        if (prepro_sp < 0 ||
            (stck_code != id_if && stck_code != id_ifdef && stck_code != id_ifndef)) {
            if (stck_code == id_else) {
                compile_error("#else after #else");
            } else {
                compile_error("#else without #if");
    ...
    if (token.code == id_endif) {
        ...
        if (prepro_sp < 0 || (stck_code != id_else && stck_code != id_if && stck_code != id_ifdef &&
                              stck_code != id_ifndef))
            compile_error("#endif without #if");
...
void token_dump() {
    if (token.type == NUMBER)
        fprintf(stderr, "%d", token.code);
    else if (token.type == FLOAT)
        fprintf(stderr, "%f", token.value);
    else if (token.type == STRING)
        fprintf(stderr, "\"%s\"", string_get(token.code));
    else if (token.type == IDENTIFIER)
        fprintf(stderr, "\"%s\"", identifier_name(token.code));
    else if (token.type == LABEL)
        fprintf(stderr, "\"%s\"", identifier_name(token.code));
    else if (token.type == NOTOKEN)
        fprintf(stderr, "EOF");
    else
...
void token_next() {
    ...
    while (1) {
        ...
        if (*source_ptr == '<') {
            ...
            if (source_ptr[1] == '=')
                len = 2;
            else if (strchr("+-&^", *source_ptr) && source_ptr[1] == *source_ptr)
                len = 2;
            else
                len = 1;
        }
        ...
        if (ISNUM(*source_ptr)) {
            ...
            while (ISNUM(*source_ptr) || (base > 10 && ISALNUM(*source_ptr))) {
                if (base == 2 && *source_ptr != '0' && *source_ptr != '1')
                    break;
                if (base == 8 && (*source_ptr < '0' || *source_ptr > '7'))
                    break;
                if (base == 10 && !ISNUM(*source_ptr))
                    break;
                if (base == 16 && !ISNUM(*source_ptr) &&
                    (TOUPPER(*source_ptr) < 'A' || TOUPPER(*source_ptr) > 'F'))
                    break;
            ...
            if (base == 16 && (*source_ptr == 'h' || *source_ptr == 'H'))
                source_ptr++;
            if (base == 8 && (*source_ptr == 'o' || *source_ptr == 'O'))
                source_ptr++;
            if (base == 2 && (*source_ptr == 'b' || *source_ptr == 'B'))
                source_ptr++;
