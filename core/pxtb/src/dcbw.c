...
void dcb_add_file(const char *filename) {
    ...
    /*
        if ( strlen( filename ) > 4 && !strncmp( filename + strlen( filename ) - 4, ".dll", 4 ) )
       return;
        if ( strlen( filename ) > 3 && !strncmp( filename + strlen( filename ) - 3, ".so", 3 ) )
       return;
        if ( strlen( filename ) > 6 && !strncmp( filename + strlen( filename ) - 6, ".dylib", 6 ) )
       return;
    */
    ...
    for (i = 0; i < dcb_filecount; i++)
        if (strcmp(filename, (const char *)dcb_files[i].Name) == 0)
            return;
...
static int dcb_varspace(VARSPACE *v) {
    ...
    dcb.varspace =
        (DCB_VARSPACE *)realloc(dcb.varspace, sizeof(DCB_VARSPACE) * (dcb_varspaces + 1));
    dcb_orig_varspace =
        (VARSPACE **)realloc(dcb_orig_varspace, sizeof(VARSPACE *) * (dcb_varspaces + 1));

    dcb_orig_varspace[dcb_varspaces] = v;

    dcb.varspace[dcb_varspaces].NVars = v->count;
    ARRANGE_DWORD(&dcb.varspace[dcb_varspaces].NVars); /* Used Only for dcb */
...
int dcb_save(const char *filename, int options, const char *stubname) {
    ...
    dcb.data.NSysProcsCodes = NSysProcs;
    ...
    for (n = 0; n < procdef_count; n++) {
        ...
        dcb.proc[n].data.OErrorCode = procs[n]->errorcode;
        ...
        dcb.proc[n].sentence =
            (DCB_SENTENCE *)calloc(procs[n]->sentence_count, sizeof(DCB_SENTENCE));

        for (i = 0; i < procs[n]->sentence_count; i++) {
            dcb.proc[n].sentence[i].NFile = 0;
            ARRANGE_DWORD(&dcb.proc[n].sentence[i].NFile);
            dcb.proc[n].sentence[i].NLine = procs[n]->sentences[i].line;
            ARRANGE_DWORD(&dcb.proc[n].sentence[i].NLine);
            dcb.proc[n].sentence[i].NCol = procs[n]->sentences[i].col;
            ARRANGE_DWORD(&dcb.proc[n].sentence[i].NCol);
            dcb.proc[n].sentence[i].OCode = procs[n]->sentences[i].offset;
            ARRANGE_DWORD(&dcb.proc[n].sentence[i].OCode);
        }
        ...
        for (i = 0; i < procs[n]->privars->count; i++) {
            ...
            ARRANGE_DWORD(&dcb.proc[n].privar[i].ID);
            dcb.proc[n].privar[i].Offset = procs[n]->privars->vars[i].offset;
            ARRANGE_DWORD(&dcb.proc[n].privar[i].Offset);
        }
        ...
        for (i = 0; i < procs[n]->pubvars->count; i++) {
            ...
            ARRANGE_DWORD(&dcb.proc[n].pubvar[i].ID);
            dcb.proc[n].pubvar[i].Offset = procs[n]->pubvars->vars[i].offset;
            ARRANGE_DWORD(&dcb.proc[n].pubvar[i].Offset);
        }
    }
    ...
    for (n = 0; n < global.count; n++) {
        ...
        ARRANGE_DWORD(&dcb.glovar[n].ID);
        dcb.glovar[n].Offset = global.vars[n].offset;
        ARRANGE_DWORD(&dcb.glovar[n].Offset);
    }
    ...
    for (n = 0; n < local.count; n++) {
        ...
        ARRANGE_DWORD(&dcb.locvar[n].ID);
        dcb.locvar[n].Offset = local.vars[n].offset;
        ARRANGE_DWORD(&dcb.locvar[n].Offset);
    }
    ...
    dcb.data.OProcsTab = offset;
    offset += sizeof(DCB_PROC_DATA) * procdef_count;
    ARRANGE_DWORD(&dcb.data.OProcsTab);
    dcb.data.OStrings = offset;
    offset += 4 * string_count;
    ARRANGE_DWORD(&dcb.data.OStrings);
    dcb.data.OGloVars = offset;
    offset += sizeof(DCB_VAR) * global.count;
    ARRANGE_DWORD(&dcb.data.OGloVars);
    dcb.data.OLocVars = offset;
    offset += sizeof(DCB_VAR) * local.count;
    ARRANGE_DWORD(&dcb.data.OLocVars);
    dcb.data.OLocStrings = offset;
    offset += 4 * local.stringvar_count;
    ARRANGE_DWORD(&dcb.data.OLocStrings);
    dcb.data.OID = offset;
    offset += sizeof(DCB_ID) * identifier_count;
    ARRANGE_DWORD(&dcb.data.OID);
    dcb.data.OVarSpaces = offset;
    offset += sizeof(DCB_VARSPACE) * dcb_varspaces;
    ARRANGE_DWORD(&dcb.data.OVarSpaces);
    dcb.data.OText = offset;
    offset += string_used;
    ARRANGE_DWORD(&dcb.data.OText);
    dcb.data.OImports = offset;
    offset += 4 * nimports;
    ARRANGE_DWORD(&dcb.data.OImports);
    dcb.data.OGlobal = offset;
    offset += globaldata->current;
    ARRANGE_DWORD(&dcb.data.OGlobal);
    dcb.data.OLocal = offset;
    offset += localdata->current;
    ARRANGE_DWORD(&dcb.data.OLocal);

    dcb.data.OSourceFiles = offset;
    ...
    dcb.data.OSysProcsCodes = offset;
    ...
    for (n = 0; n < dcb_varspaces; n++) {
        dcb.varspace[n].OVars = offset;
        ARRANGE_DWORD(&dcb.varspace[n].OVars);
        offset += sizeof(DCB_VAR) * dcb_orig_varspace[n]->count;
    }

    for (n = 0; n < procdef_count; n++) {
        dcb.proc[n].data.OSentences = offset;
        offset += sizeof(DCB_SENTENCE) * procs[n]->sentence_count;
        ARRANGE_DWORD(&dcb.proc[n].data.OSentences);

        /* Private */
        dcb.proc[n].data.OPriVars = offset;
        offset += sizeof(DCB_VAR) * procs[n]->privars->count;
        ARRANGE_DWORD(&dcb.proc[n].data.OPriVars);
        dcb.proc[n].data.OPriStrings = offset;
        offset += 4 * procs[n]->privars->stringvar_count;
        ARRANGE_DWORD(&dcb.proc[n].data.OPriStrings);
        dcb.proc[n].data.OPrivate = offset;
        offset += procs[n]->pridata->current;
        ARRANGE_DWORD(&dcb.proc[n].data.OPrivate);

        /* Publics */
        dcb.proc[n].data.OPubVars = offset;
        offset += sizeof(DCB_VAR) * procs[n]->pubvars->count;
        ARRANGE_DWORD(&dcb.proc[n].data.OPubVars);
        dcb.proc[n].data.OPubStrings = offset;
        offset += 4 * procs[n]->pubvars->stringvar_count;
        ARRANGE_DWORD(&dcb.proc[n].data.OPubStrings);
        dcb.proc[n].data.OPublic = offset;
        offset += procs[n]->pubdata->current;
        ARRANGE_DWORD(&dcb.proc[n].data.OPublic);

        /* Code */
        dcb.proc[n].data.OCode = offset;
        offset += procs[n]->code.current * 4;
        ARRANGE_DWORD(&dcb.proc[n].data.OCode);
    }

    /* Archivos incluidos */
    OFilesTab          = offset;
    dcb.data.OFilesTab = OFilesTab;
    ...
    for (n = 0; n < dcb_filecount; n++) {
        dcb_fullname[n] = (char *)dcb.file[n]
                              .Name; /* Guardo el Name, porque lo destruyo en SName (es un union) */
        dcb.file[n].SName =
            strlen((const char *)dcb.file[n].Name) +
            1; /* Todavia no hago el ARRANGE de este dato, lo hago luego cuando lo voy a grabar */
    ...
    file_write(fp, dcb.varspace,
               sizeof(DCB_VARSPACE) * dcb_varspaces); /* Ya procesado el byteorder */
    ...
    for (n = 0; n < procdef_count; n++) {
        file_write(fp, dcb.proc[n].sentence,
                   sizeof(DCB_SENTENCE) * procs[n]->sentence_count); /* Ya procesado el byteorder */

        /* Privadas */
        file_write(fp, dcb.proc[n].privar,
                   sizeof(DCB_VAR) * procs[n]->privars->count); /* Ya procesado el byteorder */
        file_writeUint32A(fp, (uint32_t *)procs[n]->privars->stringvars,
                          procs[n]->privars->stringvar_count);
        file_write(fp, procs[n]->pridata->bytes, procs[n]->pridata->current); /* ****** */

        /* Publicas */
        file_write(fp, dcb.proc[n].pubvar,
                   sizeof(DCB_VAR) * procs[n]->pubvars->count); /* Ya procesado el byteorder */
        file_writeUint32A(fp, (uint32_t *)procs[n]->pubvars->stringvars,
                          procs[n]->pubvars->stringvar_count);
...
void dcb_varspaces_load(file *fp, VARSPACE *vs, VARSPACE *vs_array, int count) {
    ...
    for (n = 0; n < count; n++) {
        ...
        for (m = 0; m < MAX_TYPECHUNKS; m++)
            ARRANGE_DWORD(&vars.Type.Count[m]);
        ...
        for (m = 0; m < MAX_TYPECHUNKS; m++) {
            ...
            if (type.chunk[m].type != TYPE_UNDEFINED)
                type.depth++;
        }
...
void dcb_vars_load(file *fp, VARSPACE *vs, segment *data, char *data_source, int count,
                   range **offs, int totalsize) {
    int n, m;
    ...
    if (offs)
        *offs = calloc(count, sizeof(range));
    ...
    for (n = 0; n < count; n++) {
        ...
        for (m = 0; m < MAX_TYPECHUNKS; m++)
            ARRANGE_DWORD(&vars[n].Type.Count[m]);
...
int dcb_load_lib(const char *filename) {
    ...
    if (memcmp(dcb.data.Header, DCL_MAGIC, sizeof(DCL_MAGIC) - 1) != 0)
        return 0;
    ...
    if (dcb.data.NStrings) {
        ...
        for (n = 0; n < dcb.data.NStrings; n++)
            stringid[n] = string_new(&strdata[stroffs[n]]);
    ...
    if (dcb.data.NImports) {
        ...
        for (n = 0; n < dcb.data.NImports; n++)
            import_mod((char *)string_get(stringid[dcb.imports[n]]));
        FREEM(dcb.imports);
    }
    ...
    if (dcb.data.NGloVars) {
        ...
        dcb_vars_load(fp, &global, globaldata, glodata, dcb.data.NGloVars, &glovaroffs,
                      dcb.data.SGlobal);
    }

    if (dcb.data.NLocVars) {
        file_seek(fp, dcb.data.OLocVars, SEEK_SET);
        dcb_vars_load(fp, &local, localdata, locdata, dcb.data.NLocVars, &locvaroffs,
                      dcb.data.SLocal);
    }
    ...
    if (dcb.data.NLocStrings) {
        ...
        for (m = 0; m < dcb.data.NLocStrings; m++) {
            ...
            for (found = 0, o = 0; o < local.stringvar_count; o++)
                if (newoff == local.stringvars[o]) {
                    found = 1;
                    break;
                }
            if (!found)
                varspace_varstring(&local, newoff);
        }
        FREEM(locstr);
    }
    ...
    for (n = 0; n < dcb.data.NProcs; n++) {
        ...
        file_readUint32A(fp, (uint32_t *)proc->code.data,
        ...
        if (dcb.proc[n].data.NPriVars) {
            ...
            dcb_vars_load(fp, proc->privars, proc->pridata, pridata, dcb.proc[n].data.NPriVars,
                          &prioffs, dcb.proc[n].data.SPrivate);
        }
        ...
        if (dcb.proc[n].data.NPubVars) {
            ...
            dcb_vars_load(fp, proc->pubvars, proc->pubdata, pubdata, dcb.proc[n].data.NPubVars,
                          &puboffs, dcb.proc[n].data.SPublic);
        }
        ...
        if (dcb.proc[n].data.NPriStrings) {
            ...
            for (m = 0; m < dcb.proc[n].data.NPriStrings; m++)
                varspace_varstring(proc->privars,
                                   get_new_off(sv[m], prioffs, dcb.proc[n].data.NPriVars));
            FREEM(sv);
        }
        ...
        if (dcb.proc[n].data.NPubStrings) {
            ...
            for (m = 0; m < dcb.proc[n].data.NPubStrings; m++)
                varspace_varstring(proc->pubvars,
                                   get_new_off(sv[m], puboffs, dcb.proc[n].data.NPubVars));
            FREEM(sv);
        }
