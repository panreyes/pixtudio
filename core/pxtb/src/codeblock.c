...
void codeblock_postprocess(CODEBLOCK *code) {
    ...
    while (ptr < code->data + code->current) {
        ...
        if (!my->imported) {
            if (*ptr == MN_JUMP || *ptr == MN_NCALL || *ptr == MN_JFALSE || *ptr == MN_JTFALSE ||
                *ptr == MN_JTRUE || *ptr == MN_JTTRUE || *ptr == MN_JNOCASE || *ptr == MN_CLONE ||
...
static void codeblock_label_alloc(CODEBLOCK *c, int count) {
    ...
    c->labelsextra = (struct _labelsextra *)realloc(
        c->labelsextra, c->label_reserved * sizeof(struct _labelsextra));
    if (!c->labels) {
        fprintf(stdout, "CODEBLOCK: out of memory\n");
        exit(1);
    }
...
void codeblock_add(CODEBLOCK *c, int code, int param) {
    ...
    if (c->current + 2 >= c->reserved)
        codeblock_alloc(c, 32);
}
