        ...
        */
        if (token.type == STRING && *t == TYPE_CHAR) {
            ...
            remaining -= subsize;
                   ...
                   (token.code == identifier_rightp || token.code == identifier_semicolon)) {
            token_back();
            break;
...
int compile_struct_data(VARSPACE *n, segment *data, int size, int sub) {
    ...
    for (;;) {
        ...
        if (token.type == IDENTIFIER && token.code == identifier_leftp) {
            ...
            token_next();
            if ((elements % n->count) == 0 && size > 0 && token.type == IDENTIFIER &&
        ...
        for (;;) {
            ...
            } else /* Next variable is a single type */
            {
                ...
                segment_add_as(data, typedef_base(next_type) == TYPE_FLOAT ? *(int *)&res.fvalue
                                                                           : res.value,
                               typedef_base(next_type));
            }
 ...
 *      additive        1 if the compiling is additive (the struct members have non-local offset;
 * GLOBAL case)
 *                      or not (STRUCT case)
...
int compile_varspace(VARSPACE *n, segment *data, int additive, int copies, int padding,
                     VARSPACE **collision, int alignment, int duplicateignore) {
    ...
    for (;;) {
        ...
        if ((var = varspace_search(n, token.code))) {
            if (duplicateignore) {
                ...
                for (;;) {
                    ...
                    /* Se salta todo hasta el puntos y coma o en el end o la coma si no hay
                     * asignacion */
        ...
        if (!segm && typedef_is_struct(type)) {
            ...
            i--;
            for (j = 0; j <= i; j++)
                typeaux.chunk[j] = type.chunk[i - j];
            for (j = 0; j <= i; j++)
                type.chunk[j] = typeaux.chunk[j];
            ...
            continue; /* No ; */
                   ...
                   token.code == identifier_leftb) /* Compila un array */
        {
            total_count = 1;
        ...
        } else if (segm && token.type == IDENTIFIER &&
                   token.code ==
                       identifier_equal) /* Compila una asignación de valores por defecto */
        {
            segment_add_from(data, segm);
            i             = data->current;
            data->current = n->vars[n->count].offset;
