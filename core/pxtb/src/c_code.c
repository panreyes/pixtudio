...
static int check_numeric_types(expresion_result *left, expresion_result *right) {
    ...
    if (typedef_base(left->type) == TYPE_CHAR) {
        ...
        if (typedef_base(right->type) == TYPE_CHAR || typedef_is_integer(right->type))
            return MN_BYTE;
    }
    ...
    if (typedef_base(right->type) == TYPE_CHAR) {
        ...
        if (typedef_base(left->type) == TYPE_CHAR || typedef_is_integer(left->type))
            return MN_BYTE;
    }
...
static int check_numeric_or_string_types(expresion_result *left, expresion_result *right) {
    if (typedef_is_array(left->type) && left->type.chunk[1].type == TYPE_CHAR &&
        typedef_is_string(right->type)) {
...
expresion_result compile_sublvalue(VARSPACE *from, int base_offset, VARSPACE *remote) {
    ...
    if (var->offset - base_offset != 0 || here == &global || here == &local ||
        (privars && (here == privars)) || (pubvars && (here == pubvars)) ||
        (remote && (here == remote))) {
        codeblock_add(code, ((remote && (here == remote))
                                 ? MN_REMOTE_PUBLIC
                                 : here == &global
                                       ? MN_GLOBAL
                                       : ((here == &local) && (here == from))
                                             ? MN_REMOTE
                                             : here == &local ? MN_LOCAL
        ...
        if ((pubvars && (here == pubvars)) ||
            (remote && (here == remote))) /* Tambien las locales remotas ? */
        {
...
int compile_sizeof(VARSPACE *here, int *content_size, char *content_type, int *parent_count) {
    ...
    if (!content_type)
        content_type = _content_type = (char *)calloc(MAX_EXPR_LEVEL, sizeof(int));
    if (!parent_count)
    ...
    if (token.type != IDENTIFIER && (token.code == identifier_unsigned ||
                                     token.code == identifier_signed)) /* "UNSIGNED" or "SIGNED" */
    {
        check_datatype = 1;
        token_next();
    }
        ...
        if (token.type == IDENTIFIER && token.code != identifier_point) {
            ...
            for (;;) {
                ...
                if (token.type == IDENTIFIER && (token.code == identifier_pointer ||
                                                 token.code == identifier_multiply)) /* "POINTER" */
                {
                    base = 4;
                    continue;
                }
    ...
    while (token.type == IDENTIFIER &&
           (token.code == identifier_multiply || token.code == identifier_leftb ||
            token.code == identifier_leftp)) {
        if (*content_size == MAX_EXPR_LEVEL)
            compile_error(MSG_TOO_COMPLEX);
    ...
    if (token.type == IDENTIFIER && token.code == identifier_leftb &&
        (typedef_is_pointer(type) ||
         typedef_is_string(type))) /* Indexado de punteros ptr[0] o cadenas */
    {
    ...
    while (token.type == IDENTIFIER &&
           (token.code == identifier_rightb || token.code == identifier_rightp)) {
        if (token.code == identifier_rightp) /* ')' */
        {
            if (!*content_size || !*parent_count)
                break;

            while ((*content_size) &&
                   content_type[(*content_size) - 1] ==
                       identifier_multiply) /* only '*', '[]' must be already process */
            {
                if (!typedef_is_pointer(type))
            ...
            if (content_type[--(*content_size)] != identifier_leftp)
                compile_error(MSG_INVALID_EXP);
            (*parent_count)--;
            token_next();
        } else {
            if (!*content_size)
                compile_error(MSG_EXTRA_CHAR);
    ...
    if (token.type == IDENTIFIER && token.code == identifier_point) /* "." */
    {
        if (typedef_is_pointer(type))
            type = typedef_reduce(type);
        if (!typedef_is_struct(type) && typedef_base(type) != TYPE_DWORD &&
            typedef_base(type) != TYPE_INT /* Soporte de process type para publicas */
            )
    ...
    /*   Process "*ptr|**ptr|***ptr|..."      */
    while ((*content_size)--) {
        if (content_type[*content_size] != identifier_multiply)
            compile_error(MSG_INVALID_EXP);
        if (!typedef_is_pointer(type))
            compile_error(MSG_NOT_A_POINTER);
        type = typedef_reduce(type);
...
expresion_result compile_cast() {
    ...
    while (token.type == IDENTIFIER && (token.code == identifier_pointer ||
                                        token.code == identifier_multiply)) /* "POINTER" or "*" */
    {
    ...
    if (typedef_is_pointer(type)) {
        ...
        if (typedef_is_float(res.type)) {
            ...
            codeblock_add(code, MN_STR2INT, 0);
            res.type = typedef_new(TYPE_INT);
        } else if (typedef_is_integer(res.type) || typedef_is_pointer(res.type) ||
                   typedef_base(res.type) == TYPE_CHAR) {
            if (res.lvalue) {
                codeblock_add(code, mntype(res.type, 0) | MN_PTR, 0);
                res.lvalue = 0;
            }
...
expresion_result compile_value() {
    ...
    if (token.type == IDENTIFIER) {
        ...
        if (token.code == identifier_offset ||
            token.code == identifier_bandoffset) /* "OFFSET" or "&" */
        {
...
expresion_result compile_factor() {
    ...
    if (token.type == IDENTIFIER) {
        ...
        } else if (token.code == identifier_bnot) /* "BNOT" or "~" */
        {
            ...
            if (mntype(part.type, 0) == MN_FLOAT)
                compile_error(MSG_INTEGER_REQUIRED);
    ...
    for (;;) {
            ...
            if (token.code == identifier_point) /* "." */ /* Operador "." */
            {
                ...
                if (typedef_is_struct(part.type)) {
                    ...
                    if (typedef_base(part.type) != TYPE_DWORD &&
                        typedef_base(part.type) != TYPE_INT)
                        compile_error(MSG_INTEGER_REQUIRED);
                    if (part.lvalue)
                        codeblock_add(code, mntype(part.type, 0) | MN_PTR, 0);

                    part = compile_sublvalue(&local, 0,
                                             v); /* referenciada REMOTA por proceso (Splinter) */
                }
                continue;
        ...
        if (token.type == IDENTIFIER && token.code == identifier_leftb) /* "[" */
        {
            ...
            {
                if (part.lvalue)
                    codeblock_add(code, MN_STRING | MN_PTR, 0);
...
expresion_result compile_operation() {
    ...
    for (;;) {
        ...
        if (typedef_is_array(left.type) && left.lvalue && token.type == IDENTIFIER &&
            token.code == identifier_plus && left.type.chunk[1].type == TYPE_CHAR) /* "+" */
        {
            codeblock_add(code, MN_A2STR, 0);
            left.lvalue = 0;
            left.type   = typedef_new(TYPE_STRING);
        ...
        if (token.type == IDENTIFIER &&
            (token.code == identifier_plus || token.code == identifier_minus)) /* "+" or "-" */
        {
            op = token.code == identifier_plus ? MN_ADD : MN_SUB;
            if (left.lvalue)
                codeblock_add(code, mntype(left.type, 0) | MN_PTR, 0);
...
expresion_result compile_rotation() {
    ...
    for (;;) {
        token_next();
        if (token.type == IDENTIFIER &&
            (token.code == identifier_ror || token.code == identifier_rol)) /* ">>" or "<<" */
        {
...
expresion_result compile_comparison_1() {
    ...
    for (;;) {
        token_next();
        if (token.type == IDENTIFIER && (token.code == identifier_gt ||  /* ">" */
                                         ...
                                         token.code == identifier_lte))  /* "<=" or "=<" */
        {
            ...
            if (right.lvalue &&
                (right.type.chunk[0].type != TYPE_ARRAY || right.type.chunk[1].type != TYPE_CHAR))
                codeblock_add(code, mntype(right.type, 0) | MN_PTR, 0);
...
expresion_result compile_bitwise_and() {
    ...
    for (;;) {
        token_next();
        if (token.type == IDENTIFIER && (token.code == identifier_band ||
                                         token.code == identifier_bandoffset)) /* "BAND" or "&" */
        {
            if (left.lvalue)
                codeblock_add(code, mntype(left.type, 0) | MN_PTR, 0);
...
expresion_result compile_logical_and() {
    ...
    for (;;) {
        ...
        if (token.type == IDENTIFIER && token.code == identifier_and) /* "AND" or "&&" */
        {
            ...
            if (left.lvalue)
                codeblock_add(code, MN_PTR | mntype(left.type, 0), 0);
            check_integer_type(&left);
            codeblock_add(code, MN_JTFALSE, et1);
            right = compile_bitwise_or();
            if (right.lvalue)
                codeblock_add(code, MN_PTR | mntype(right.type, 0), 0);
...
expresion_result compile_ternarycond() {
    ...
    {
        if (base.lvalue)
            codeblock_add(code, mntype(base.type, 0) | MN_PTR, 0);
    ...
    if (token.type == IDENTIFIER) {
        ...
        if (token.code == identifier_equal) /* "=" */
        {
            if (typedef_is_array(base.type) && base.lvalue &&
                base.type.chunk[1].type == TYPE_CHAR) /* Asignaciones a cadenas de ancho fijo */
            {
                right = compile_expresion(0, 0, 0, TYPE_UNDEFINED);
                if (typedef_is_integer(right.type)) {
            ...
            if (typedef_is_pointer(base.type)) /* Asignaciones a punteros */
            {
                ...
                if ((typedef_base(right.type) == TYPE_DWORD ||
                     typedef_base(right.type) == TYPE_INT) &&
        ...
        if (typedef_is_pointer(base.type) &&
            (token.code == identifier_plusequal ||
             token.code == identifier_minusequal)) /* "+=" or "-=" */
        {
            TYPEDEF pointer_type;
        ...
        {
            if (typedef_is_array(base.type) && base.lvalue &&
                base.type.chunk[1].type == TYPE_CHAR) /* Cadena += cadena fija */
            {
                right = compile_expresion(0, 0, 0, TYPE_UNDEFINED);
                if (typedef_is_integer(right.type)) {
                    compile_warning(1, "implicit conversion (INTEGER to CHAR[])");
                ...
                if (!typedef_is_struct(typedef_reduce(right.type))) {
                    ...
                    if (typedef_base(base.type) != TYPE_STRUCT)
                        ...
                        if (right.type.varspace->stringvar_count > 0) {
                            ...
                            codeblock_add(code, MN_PUSH, right.type.varspace->size *
...
expresion_result compile_expresion(int need_constant, int need_lvalue, int discart_code,
                                   BASETYPE t) {
    expresion_result res;
    ...
    if (typedef_base(res.type) == TYPE_ARRAY && res.type.chunk[1].type == TYPE_CHAR && res.lvalue &&
        !need_lvalue) {
        codeblock_add(code, MN_A2STR, 0);
...
expresion_result convert_result_type(expresion_result res, BASETYPE t) {
    ...
    } else if (typedef_is_integer(res.type) /*&& res.constant && res.value == 0*/ &&
               t == TYPE_POINTER) {
    ...
    } else if ((t == TYPE_BYTE || t == TYPE_WORD || t == TYPE_DWORD) &&
...
void compile_block(PROCDEF *p) {
    ...
    for (;;) {
        ...
        if (token.type == IDENTIFIER) {
            ...
            if (token.code == identifier_continue) /* "CONTINUE" */
            {
                if (dcb_options & DCB_DEBUG)
                    codeblock_add(code, MN_SENTENCE, line_count + (current_file << 20));
                ...
                for (;;) {
                    ...
                    if (token.type != IDENTIFIER || token.code != identifier_leftp) /* "(" */
                    {
                        ...
                        if (token.type != IDENTIFIER ||
                            (token.code != identifier_semicolon &&
                             token.code != identifier_colon)) /* ";" or ":" */
                               ...
                               token.code == identifier_elseif) /* "ELSEIF" */
                    {
                        if (dcb_options & DCB_DEBUG)
                            codeblock_add(code, MN_SENTENCE, line_count + (current_file << 20));
                        if (end_of_chain == -1)
                            end_of_chain = codeblock_label_add(code, -1);
                ...
                for (;;) {
                    ...
                    if (token.type == IDENTIFIER && token.code == identifier_case) /* "CASE" */
                    {
                        ...
                        for (;;) {
                            ...
                            token_next();
                            if (token.type == IDENTIFIER &&
                                token.code == identifier_twopoints) /* ".." */
                            {
                                compile_expresion(0, 0, 0, switch_type);
                                if (switch_type == TYPE_STRING) {
                        ...
                        codeblock_label_set(code, et2, code->current);
                               ...
                               token.code == identifier_default) /* "DEFAULT" */
                    {
                        if (dcb_options & DCB_DEBUG)
                            codeblock_add(code, MN_SENTENCE, line_count + (current_file << 20));
            ...
            if (token.code == identifier_from) /* "FROM" */
            {
                ...
                if (!res.lvalue)
                    compile_error(MSG_VARIABLE_REQUIRED);
                if (!typedef_is_numeric(res.type))
                    compile_error(MSG_NUMBER_REQUIRED);
            ...
            if (token.code == identifier_on) /* ON EXIT/ON ERROR */
            {
                ...
                if (token.type != IDENTIFIER ||
                    (token.code != identifier_exit && token.code != identifier_error))
                    compile_error(MSG_ON_PARAM_ERR);

                on_req = token.code;

                token_next();
                if (token.type != IDENTIFIER || token.code != identifier_jmp)
                    compile_error(MSG_GOTO_EXP);
                ...
                codeblock_add(code, on_req == identifier_exit ? MN_EXITHNDLR : MN_ERRHNDLR,
                              codelabel);
        ...
        if (token.type == LABEL || /* Label */
            (token.type == IDENTIFIER && (token.code == identifier_onexit ||
                                          token.code == identifier_onerror)) /* ONEXIT or ONERROR */
            ) {
