...
void compile_error(const char *fmt, ...) {
    char text[4000];
    char *fname = (import_filename)
                      ? import_filename
                      : ((current_file != -1 && files[current_file] && *files[current_file])
                             ? files[current_file]
                             : NULL);
    ...
    fprintf(stderr, MSG_COMPILE_ERROR,
            ...
            fname ? fname : "N/A", (import_filename) ? import_line : line_count, text);
...
static int import_exists(char *libname) {
    ...
    for (n = 0; n < nimports_aux; n++)
        if (!strcmp(libname, string_get(imports_aux[n])))
            return n;
    return -1;
}
...
static char *modules_exts[] = {".dll", ".dylib", ".so", NULL};

static char *dlsearchpath[] = {"modules", "mod", "mods", "extensions", "plugins", "data", NULL};
...
#define SIZEDLLEXT 3
    ...
    if (exported_functions) {
        while (exported_functions->name) {
            sysproc_add(exported_functions->name, exported_functions->paramtypes,
                        exported_functions->type, exported_functions->func);
            exported_functions++;
        }
    }
}
...
void compile_constants() {
    ...
    for (;;) {
        ...
        if (token.code == identifier_begin || token.code == identifier_local ||
            token.code == identifier_public || token.code == identifier_private ||
        ...
        if (!typedef_is_float(res.type) && !typedef_is_string(res.type) &&
            !typedef_is_integer(res.type)) {
            compile_error(MSG_INVALID_TYPE);
        }

        constants_add(code, res.type,
                      typedef_base(res.type) == TYPE_FLOAT ? *(int *)&res.fvalue : res.value);
...
void compile_process() {
    ...
    if ((tcode == identifier_process ||
         tcode == identifier_function)) { /* If it's a process or function, obtain the sign */
        token_next();
        tcode = token.code;
    }
    ...
    while (token.code == identifier_pointer ||
           token.code == identifier_multiply) { /* Jump to identifier POINTER */
        tcode = token.code;
        token_next();
    }
    ...
    while (token.type != IDENTIFIER || token.code != identifier_rightp) {
        ...
        if (token.type == IDENTIFIER && token.code == identifier_dword) {
            ...
            external_proc = NULL;
            token_next();
        } else if (!external_proc &&
                   (external_proc = procdef_search(token.code))) { /* process type var, Splinter */
            type_implicit = 0;
            type          = TYPE_INT;
            ctype = typedef_new(type);
            token_next();
        ...
        while (token.type == IDENTIFIER &&
               (token.code == identifier_pointer || token.code == identifier_multiply)) {
            type_implicit = 0;
            type          = TYPE_POINTER;
        ...
        if (!proc->declared) {
            ...
            if (var) {
                ...
                if (var) {
                    ...
                    if (external_proc) {
                        proc->privars->vars[proc->privars->count].type.varspace =
                            external_proc->pubvars;
                    }
        ...
        if (proc->params != -1) {
            ...
            if (proc->paramtype[params] == TYPE_UNDEFINED) {
                proc->paramtype[params] = type;
                        ...
                        proc->paramtype[params] == TYPE_INT) &&
                       (type == TYPE_DWORD || type == TYPE_INT || type == TYPE_BYTE ||
                        type == TYPE_WORD || type == TYPE_SHORT || type == TYPE_SBYTE ||
                        type == TYPE_FLOAT)) {
                // Parameter type was implicitly defined to some other compatible type
                proc->paramtype[params] = type;
            } else if (type == TYPE_DWORD && (proc->paramtype[params] == TYPE_BYTE ||
                                              proc->paramtype[params] == TYPE_WORD)) {
                proc->paramtype[params] = type;
    ...
    while (token.type == IDENTIFIER &&
           (token.code == identifier_local || token.code == identifier_public ||
            token.code == identifier_private)) {
        if ((!proc->declared) &&
            (token.code == identifier_local || token.code == identifier_public)) {
            /* (2006/11/19 19:34 GMT-03:00, Splinter - jj_arg@yahoo.com) */
            /* Ahora las declaraciones locales, son solo locales al proceso, pero visibles desde
             * todo proceso */
            /* Se permite declarar local/publica una variable que haya sido declarada global, es una
             * variable propia, no es la global */
            ...
            /* (2006/11/19 19:34 GMT-03:00, Splinter - jj_arg@yahoo.com) */
            /* Se permite declarar privada una variable que haya sido declarada global, es una
             * variable propia, no es la global */
    ...
    for (;;) {
        ...
        if (token.type == IDENTIFIER && token.code == identifier_import) {
            ...
            compile_type(); /* Tipo de dato definido por el usuario */
                   ...
                   (token.code == identifier_process || token.code == identifier_function ||
                    token.code == identifier_declare || identifier_is_basic_type(token.type))) {
