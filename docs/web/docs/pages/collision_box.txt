...
**INT** collision\_box ( &lt;**INT** processID|processTypeID&gt; )

Checks if a [process](process "wikilink") collided with the process
calling Collision\_box().

When a [processTypeID](processTypeID "wikilink") is specified, there
could be multiple fitting collisions. In this case, collision\_box()
...
The diference between collision\_box and
[collision](collision "wikilink") is that collision\_box only checks the
distance with the edges of the processes, on the other side collision()
check pixel by pixel between these processes. Consequently,
collision\_box() is faster, but is more accurate collision().
...
[key](key "wikilink")(), [collision\_box](collision_box "wikilink")(),
