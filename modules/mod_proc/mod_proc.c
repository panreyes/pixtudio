...
enum { PROCESS_ID = 0, PROCESS_TYPE, STATUS, ID_SCAN, TYPE_SCAN, CONTEXT, SIGNAL_ACTION };
...
int modproc_running(INSTANCE *my, int *params) {
    ...
    if (params[0] >= FIRST_INSTANCE_ID) {
        ...
        if (i && (LOCDWORD(mod_proc, i, STATUS) & ~STATUS_WAITING_MASK) >= STATUS_RUNNING)
            return 1;
        return 0;
    }
    ...
    while ((i = instance_get_by_type(params[0], &ctx))) {
        if ((LOCDWORD(mod_proc, i, STATUS) & ~STATUS_WAITING_MASK) >= STATUS_RUNNING)
            return 1;
    }
...
int modproc_signal(INSTANCE *my, int *params) {
    ...
    if (params[0] == ALL_PROCESS) {
        ...
        i = first_instance;
        while (i) {
            if (LOCDWORD(mod_proc, i, PROCESS_ID) != myid &&
...
int modproc_let_me_alone(INSTANCE *my, int *params) {
    ...
    if (LOCDWORD(mod_proc, my, STATUS) > STATUS_KILLED)
        LOCDWORD(mod_proc, my, STATUS) = STATUS_RUNNING;
    return 1;
}
...
int modproc_get_id(INSTANCE *my, int *params) {
    ...
    */
    ...
    while ((ptr = instance_get_by_type(params[0], ctx))) {
        if (/*ptr != my &&*/ (LOCDWORD(mod_proc, ptr, STATUS) & ~STATUS_WAITING_MASK) >=
            STATUS_RUNNING) {
            return LOCDWORD(mod_proc, ptr, PROCESS_ID);
        }
    }
