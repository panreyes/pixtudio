...
int modeffects_filter(INSTANCE *my, intptr_t *params) {
    ...
    for (i = 0; i < map->width; i++) {
        for (j = 0; j < map->height; j++) {
            ...
            for (y = j - 1; y < j + 2; y++) {
                ...
                b2 = 0;
                for (x = i - 1; x < i + 2; x++, c++) {
                    color = _get_pixel(map, (x < 0) ? 0 : (x > map->width - 1) ? map->width - 1 : x,
                                       (y < 0) ? 0 : (y > map->height - 1) ? map->height - 1 : y);
                    if (!color) {
...
int modeffects_blur(INSTANCE *my, intptr_t *params) {
    GRAPH *map = bitmap_get(params[0], params[1]), *map2;
...
int modeffects_grayscale(INSTANCE *my, intptr_t *params) { // fpg,map,tipo
...
int modeffects_rgbscale(INSTANCE *my, intptr_t *params) {
    GRAPH *map = bitmap_get(params[0], params[1]);
    uint32_t i, j, c;
    int r, g, b, a;
    ...
    for (i = 0; i < map->height; i++) {
        for (j = 0; j < map->width; j++) {
            ...
            c = (int)(0.3 * r + 0.59 * g + 0.11 * b);
            c = gr_rgba_depth(map->format->depth, (int)(c * *(float *)(&params[2])),
                              (int)(c * *(float *)(&params[3])), (int)(c * *(float *)(&params[4])),
                              a);
            _put_pixel(map, j, i, c);
        }
    }
