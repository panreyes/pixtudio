...
_inline void _HLine16_nostipple(uint16_t *ptr, uint32_t length) {
    ...
    if (pixel_alpha == 255) {
        ...
        for (n = length; n >= 0; n--, ptr++)
            *ptr = pixel_alpha16[*ptr] + pixel_color16_alpha;
    }
}
...
void draw_rectangle(GRAPH *dest, REGION *clip, int x, int y, int w, int h) {
    ...
    if (h)
        draw_vline(dest, clip, x + w, y, h - ih);
    if (w && h)
        draw_hline(dest, clip, x + w, y + h, -(w - iw));
    if (h && w)
        draw_vline(dest, clip, x, y + h, -(h - ih));
...
void draw_bezier(GRAPH *dest, REGION *clip, int x1, int y1, int x2, int y2, int x3, int y3, int x4,
    ...
    for (i = 0; i < n; i++) {
        ...
        if ((int16_t)(xp) != (int16_t)(x) || (int16_t)(yp) != (int16_t)(y)) {
            draw_line(dest, clip, (int16_t)xp, (int16_t)yp, (int16_t)x - (int16_t)xp,
