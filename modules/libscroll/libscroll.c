...
void scroll_start(int n, int fileid, int graphid, int backid, int region, int flags, int destfile,
                  int destid) {
    ...
    if (n >= 0 && n <= 9) {
        ...
        data->reserved[0] =
            (int32_t)&scrolls[n]; /* First reserved dword point to internal scrolldata struct */
...
void scroll_update(int n) {
    ...
    if (!graph)
        return; // El fondo de scroll no existe
    if (scrolls[n].backid && !back)
        return; // Grafico no existe
    ...
    if (scrolls[n].camera) {
        ...
        if ((gr = instance_graph(scrolls[n].camera))) {
            ...
            if (scrolls[n].region1 && (x0 < scrolls[n].region1->x2 && y0 < scrolls[n].region1->y2 &&
                                       x1 > scrolls[n].region1->x && y1 > scrolls[n].region1->y)) {
                speed = 0;
            } else if (scrolls[n].region2) {
                if (x0 > scrolls[n].region2->x2)
                    speed = (x0 - scrolls[n].region2->x2);
                if (y0 > scrolls[n].region2->y2)
                    speed = (y0 - scrolls[n].region2->y2);
                if (x1 < scrolls[n].region2->x)
                    speed = (scrolls[n].region2->x - x1);
                if (y1 < scrolls[n].region2->y)
                    speed = (scrolls[n].region2->y - y1);
            }
        }
        ...
        if (data->x0 < cx)
            data->x0 = MIN(data->x0 + speed, cx);
        if (data->y0 < cy)
            data->y0 = MIN(data->y0 + speed, cy);
        if (data->x0 > cx)
            data->x0 = MAX(data->x0 - speed, cx);
        if (data->y0 > cy)
            data->y0 = MAX(data->y0 - speed, cy);
    }
    ...
    if (graph) {
        if (!(scrolls[n].flags & GRAPH_HWRAP))
            data->x0 = MAX(0, MIN(data->x0, (int)graph->width - w));
        if (!(scrolls[n].flags & GRAPH_VWRAP))
            data->y0 = MAX(0, MIN(data->y0, (int)graph->height - h));
    }
    ...
    if (back) {
        if (!(scrolls[n].flags & BACK_HWRAP))
            data->x1 = MAX(0, MIN(data->x1, (int)back->width - w));
        if (!(scrolls[n].flags & BACK_VWRAP))
            data->y1 = MAX(0, MIN(data->y1, (int)back->height - h));
    }
    ...
    if (scrolls[n].x0 < 0)
        scrolls[n].x0 += graph->width;
    if (scrolls[n].y0 < 0)
        scrolls[n].y0 += graph->height;

    if (back) {
        scrolls[n].x1 = data->x1 % (int32_t)back->width;
        scrolls[n].y1 = data->y1 % (int32_t)back->height;
        if (scrolls[n].x1 < 0)
            scrolls[n].x1 += back->width;
        if (scrolls[n].y1 < 0)
            scrolls[n].y1 += back->height;
    }
}
...
void scroll_draw(int n, REGION *clipping) {
    ...
    if (back) {
        ...
        while (y < scrolls[n].region->y2) {
            ...
            while (x < scrolls[n].region->x2) {
                gr_blit(dest, &r, x + cx, y + cy, data->flags2, data->modr2, data->modg2, data->modb2, back);
                x += back->width;
            }
            y += back->height;
        }
    ...
    while (y < scrolls[n].region->y2) {
        ...
        while (x < scrolls[n].region->x2) {
            gr_blit(dest, &r, x + cx, y + cy, data->flags1, data->modr1, data->modg1, data->modb1, graph);
            x += graph->width;
        }
        y += graph->height;
    }
