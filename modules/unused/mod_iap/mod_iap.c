/*
 *  Copyright (c) 2011-2013 Joseba GarcÃ­a Echebarria. All rights reserved.
 *
...
JNIEnv *BGD_JNI_GetEnv(void) {
     ...
     * They're usually started from managed code (using Thread.start), but they can also be created
     * elsewhere and then
     * attached to the JavaVM. For example, a thread started with pthread_create can be attached
     * with the
     * JNI AttachCurrentThread or AttachCurrentThreadAsDaemon functions. Until a thread is attached,
     * it has no JNIEnv,
     * and cannot make JNI calls.
     * Attaching a natively-created thread causes a java.lang.Thread object to be constructed and
     * added to the "main"
     * ThreadGroup, making it visible to the debugger. Calling AttachCurrentThread on an
     * already-attached thread
     * is a no-op.
     * Note: You can call this function any number of times for the same thread, there's no harm in
     * it
     */
...
static int bgd_iap_purchase(INSTANCE *my, int *params) {
    ...
    *(purchase_status) = -2;
    ...
    JNIEnv *mEnv = BGD_JNI_GetEnv();
    mid = (*mEnv)
              ->GetStaticMethodID(mEnv, mActivityClass, "purchaseProduct", "(Ljava/lang/String;)V");
    if (mid) {
        jstring prodID = (*mEnv)->NewStringUTF(mEnv, string_get(params[0]));
        string_discard(params[0]);
