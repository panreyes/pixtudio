...
int modfile_fclose(INSTANCE *my, int *params) {
    if (params[0] > 0 && params[0] <= sb_count(loaded_files) && loaded_files[params[0]-1]) {
        file_close(loaded_files[params[0]-1]);
...
int modfile_fread(INSTANCE *my, int *params) {
    if (params[0] > 0 && params[0] <= sb_count(loaded_files) && loaded_files[params[0]-1]) {
        return loadtypes(loaded_files[params[0]-1], (void *)params[1], (void *)params[2], params[3], 0);
    }
...
int modfile_fwrite(INSTANCE *my, int *params) {
    if (params[0] > 0 && params[0] <= sb_count(loaded_files) && loaded_files[params[0]-1]) {
        return savetypes(loaded_files[params[0]-1], (void *) params[1], (void *) params[2], params[3], 0);
    }
...
int modfile_freadC(INSTANCE *my, int *params) {
    if (params[2] > 0 && params[2] <= sb_count(loaded_files) && loaded_files[params[2]-1]) {
        return file_read(loaded_files[params[2]-1], (void *) params[0], params[1]);
    }
...
int modfile_fwriteC(INSTANCE *my, int *params) {
    if (params[2] > 0 && params[2] <= sb_count(loaded_files) && loaded_files[params[2]-1]) {
        return file_write(loaded_files[params[2]-1], (void *) params[0], params[1]);
    }
...
int modfile_fseek(INSTANCE *my, int *params) {
    if (params[0] > 0 && params[0] <= sb_count(loaded_files) && loaded_files[params[0]-1]) {
        return file_seek(loaded_files[params[0]-1], params[1], params[2]);
    }
...
int modfile_frewind(INSTANCE *my, int *params) {
    if (params[0] > 0 && params[0] <= sb_count(loaded_files) && loaded_files[params[0]-1]) {
        file_rewind(loaded_files[params[0]-1]);
        return 1;
    }
...
int modfile_ftell(INSTANCE *my, int *params) {
    if (params[0] > 0 && params[0] <= sb_count(loaded_files) && loaded_files[params[0]-1]) {
        return file_pos(loaded_files[params[0]-1]);
    }
...
int modfile_fflush(INSTANCE *my, int *params) {
    if (params[0] > 0 && params[0] <= sb_count(loaded_files) && loaded_files[params[0]-1]) {
        return file_flush(loaded_files[params[0]-1]);
    }
...
int modfile_filelength(INSTANCE *my, int *params) {
    if (params[0] > 0 && params[0] <= sb_count(loaded_files) && loaded_files[params[0]-1]) {
        return file_size(loaded_files[params[0]-1]);
    }
...
int modfile_fputs(INSTANCE *my, int *params) {
    if (params[0] > 0 && params[0] <= sb_count(loaded_files) && loaded_files[params[0]-1]) {
        ...
        int r = file_puts(loaded_files[params[0]-1], str);
        if (str[strlen(str) - 1] != '\n') {
            file_puts(loaded_files[params[0]-1], "\r\n");
        }
...
int modfile_fgets(INSTANCE *my, int *params) {
    ...
    if (params[0] > 0 && params[0] <= sb_count(loaded_files) && loaded_files[params[0]-1]) {
        while (!done) {
            len = file_gets(loaded_files[params[0]-1], buffer, sizeof(buffer) - 1);
            if (len < 1)
                break;
...
int modfile_feof(INSTANCE *my, int *params) {
    if (params[0] > 0 && params[0] <= sb_count(loaded_files) && loaded_files[params[0]-1]) {
        return file_eof(loaded_files[params[0]-1]);
    }
...
void __pxtexport(mod_file, module_finalize)() {
    ...
    int n = sb_count(loaded_files);
    for (int i = 0; i < n; i++) {
        file_close(loaded_files[i]);
    }
