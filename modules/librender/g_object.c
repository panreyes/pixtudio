...
void gr_destroy_object(int id) {
    ...
    /* Rects */
    if (object->bbox.x >= 0 || object->bbox.y >= 0 || object->bbox.x2 >= 0 ||
        object->bbox.y2 >= 0) {
        gr_mark_rect(object->bbox.x, object->bbox.y, object->bbox.x2 - object->bbox.x + 1,
                     object->bbox.y2 - object->bbox.y + 1);
    }
    ...
    while ((ctr = next_ctr)) {
        ...
        while ((object = next_object)) {
            ...
            if (object->seq != sequencer) {
                ...
                object->changed =
                    (*object->info)(object->what, &object->bbox, &object->z, &object->ready);
                ...
                if (object->z != key) {
                    ...
                    if (fix_ctr->first_in_key)
                        fix_ctr->first_in_key->prev = object;
                ...
                if ((object->z != key) || (object->ready ^ ready) || object->changed ||
                    object->bbox.x2 != object->bbox_saved.x2 ||
                    object->bbox.x != object->bbox_saved.x ||
                    object->bbox.y2 != object->bbox_saved.y2 ||
                    object->bbox.y != object->bbox_saved.y) {
                    if (restore == 0)
                        gr_mark_rect(object->bbox_saved.x, object->bbox_saved.y,
                                     object->bbox_saved.x2 - object->bbox_saved.x + 1,
                                     object->bbox_saved.y2 - object->bbox_saved.y + 1);

                    object->bbox_saved = object->bbox;
                }

                if (!object->ready || object->bbox.x2 < 0 || object->bbox.x > scr_width - 1 ||
                    object->bbox.y2 < 0 || object->bbox.y > scr_height - 1)
                    continue;

                if (dump == 0 && object->changed &&
                    (object->bbox.x >= 0 || object->bbox.y >= 0 || object->bbox.x2 >= 0 ||
                     object->bbox.y2 >= 0)) {
                    gr_mark_rect(object->bbox.x, object->bbox.y,
                                 object->bbox.x2 - object->bbox.x + 1,
                                 object->bbox.y2 - object->bbox.y + 1);
                }
            }
        }
    ...
    while (ctr) {
        ...
        while (object) {
            if (object->ready) {
                prect = updaterects;
                for (n = count; n--; prect++) {
                    if (object->bbox.x2 < prect->x || object->bbox.x > prect->x2 ||
