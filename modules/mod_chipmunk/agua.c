...
int modChipmunkAgua(int *ids, int tam, float kNorm, float kNearNorm, float kRestDensity,
    ...
    for (i = 0; i < tam; i++) {
        ...
        for (j = 0; j < tam; j++) {
            ...
            a        = 1 - r / KH;
            float a2 = a * a;
            d        = kDT2 * ((nearP[i] + nearP[j]) * a2 * a * kNearNorm + (p[i] + p[j]) * a2 * kNorm) /
                2;
            float rmd = d / (r * mass);
            x0 -= rmd * dx;
            y0 -= rmd * dy;
            ...
            if (u > 0) {
                u /= r;
                float It =
                    0.5 * kDT * a * (kLinearViscocity * u + kQuadraticViscocity * u * u) * kDT;
                x0 -= It * dx;
                y0 -= It * dy;
            }
        }
        cpBodySetVel(bd[i], cpv(vx[i] + (x0 * kDT), vy[i] + (y0 * kDT)));
        cpBodyUpdateVelocity(bd[i], cpv(*(float *)GLOADDR(mod_chipmunk, GLO_GRAVITY_X),
                                        *(float *)GLOADDR(mod_chipmunk, GLO_GRAVITY_Y)),
                             *(float *)GLOADDR(mod_chipmunk, GLO_DAMPING), kDT);
...
int modChipmunkEmulateAgua(INSTANCE *my, int *params) {
    ...
    if (ws->ids == 0 || ws->size > 1)
        return 0;
    modChipmunkAgua(ws->ids, ws->size, ws->kNorm, ws->kNearNorm, ws->kRestDensity, ws->kStiffness,
                    ws->kNearStiffness, kdt, ws->kSurfaceTension, ws->kLinearViscocity,
                    ws->kQuadraticViscocity, ws->KH);
    return 1;
}
