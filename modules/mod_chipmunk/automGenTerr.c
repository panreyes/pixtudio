...
#undef I
#define I(a, b, c)                                                                                 \
    (gr_get_pixel(a, b, c) != 0 && gr_get_pixel(a, b, c) != 0x00ffffff &&                          \
     gr_get_pixel(a, b, c) != 0xff000000)
GRAPH *modChipmunkObtenTodosBordes(GRAPH *mapa1) {
    ...
    if (map) {
        ...
        for (i = 1; i < anc; i++) {
            for (j = 1; j < alt; j++) {
                gr_put_pixel(map, i, j, I(mapa1, i, j) &&
                                            !(I(mapa1, i, j - 1) && I(mapa1, i - 1, j) &&
                                              I(mapa1, i + 1, j) && I(mapa1, i, j + 1)));
            }
        }
        int chi;
        for (i = 1; i < anc; i++) {
            for (j = 1; j < alt; j++) {
                if (gr_get_pixel(map, i, j) == 1) {
                    chi = (gr_get_pixel(map, i + 1, j) != gr_get_pixel(map, i, j - 1)) +
                          (gr_get_pixel(map, i, j - 1) != gr_get_pixel(map, i - 1, j)) +
                          (gr_get_pixel(map, i - 1, j) != gr_get_pixel(map, i, j + 1)) +
                          (gr_get_pixel(map, i, j + 1) != gr_get_pixel(map, i + 1, j)) +
                          2 * (((!gr_get_pixel(map, i + 1, j)) & gr_get_pixel(map, i + 1, j - 1) &
                                (!gr_get_pixel(map, i, j - 1))) +
                               ((!gr_get_pixel(map, i, j - 1)) & gr_get_pixel(map, i - 1, j - 1) &
                                (!gr_get_pixel(map, i - 1, j))) +
                               ((!gr_get_pixel(map, i - 1, j)) & gr_get_pixel(map, i - 1, j + 1) &
                                (!gr_get_pixel(map, i, j + 1))) +
                               ((!gr_get_pixel(map, i, j + 1)) & gr_get_pixel(map, i + 1, j + 1) &
                                (!gr_get_pixel(map, i + 1, j))));
                    if (chi == 2 &&
