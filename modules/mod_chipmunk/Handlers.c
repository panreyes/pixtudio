...
int modcpSpaceAddCollisionHandler(INSTANCE *my, intptr_t *params) {
    ...
    if (strlen(preSolves) != 0) {
        proc =
            procdef_get_by_name(preSolves); // Get definition function (name must be in uppercase)
        if (!proc) {
            printf("Función %s no encontrada\n", preSolves);
            modChipmunkFalloHandler(params, handler);
            return -1;
    ...
    if (strlen(postSolves) != 0) {
        proc =
            procdef_get_by_name(postSolves); // Get definition function (name must be in uppercase)
        if (!proc) {
            printf("Función %s no encontrada\n", postSolves);
            modChipmunkFalloHandler(params, handler);
            return -1;
    ...
    if (strlen(separates) != 0) {
        proc =
            procdef_get_by_name(separates); // Get definition function (name must be in uppercase)
        if (!proc) {
            printf("Función %s no encontrada\n", separates);
            modChipmunkFalloHandler(params, handler);
            return -1;
    ...
    cpSpaceAddCollisionHandler(
        modChipmunk_cpEspacio, handler->a, handler->b,
        handler->funciones[0]
            ? modChipmunkBeginCall
            : NULL, // si la string es null, poner null, de lo contrario poner beginCall
        handler->funciones[1] ? modChipmunkPreSolveCall : NULL,
        handler->funciones[2] ? modChipmunkPostSolveCall : NULL,
        handler->funciones[3] ? modChipmunkSeparateCall : NULL, (void *)handler);
    string_discard(params[2]);
    string_discard(params[3]);
    string_discard(params[4]);
    string_discard(params[5]);
...
int modcrearHandler(INSTANCE *my, intptr_t *params) {
    ...
    cpSpaceAddCollisionHandler(modChipmunk_cpEspacio, params[0], params[1], NULL, NULL,
                               recogeColisionHandler, NULL, col);
    return (int)col;
}
