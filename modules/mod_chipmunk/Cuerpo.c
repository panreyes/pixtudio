...
void addListaProcesos(INSTANCE *ins) {
    ...
    if (LOCDWORD(mod_chipmunk, ins, LOC_STATIC)) {
        ...
        a = cpBodyNew(*(float *)LOCADDR(mod_chipmunk, ins, LOC_MASS),
                      *(float *)LOCADDR(mod_chipmunk, ins, LOC_INERTIA));
...
int modaddCircleShape(INSTANCE *my, int *params) {
    ...
    cpShape *sha = cpCircleShapeNew(bod, *(float *)&params[2],
    ...
    if (LOCDWORD(mod_chipmunk, my, LOC_STATIC)) {
        cpSpaceReindexShapesForBody(modChipmunk_cpEspacio,
                                    (cpBody *)LOCDWORD(mod_chipmunk, my, LOC_BODY));
    }
    return (int)sha;
}
...
int modaddSegmentShape(INSTANCE *my, int *params) {
    ...
    DataPointer dato = (DataPointer)bod->data;
    cpShape *sha =
        cpSegmentShapeNew(bod, cpv(*(float *)&params[0], *(float *)&params[1]),
...
int modaddPolyShape(INSTANCE *my, int *params) {
    ...
    cpShape *sha =
        cpPolyShapeNew(bod, params[2], params[3], cpv(*(float *)&params[0], *(float *)&params[1]));
    //    if (dato->nShapes==0)
    //    {
    //        dato->nShapes++;
    //        dato->Shapes=malloc(sizeof(cpShape*));
...
int calculaCuerpoConvexo(GRAPH *map, modChipmunkStruct_Point *r) {
    ...
    int first;
    int cantCH =
        cpConvexHull(cant, &p, NULL, &first, *(float *)GLOADDR(mod_chipmunk, GLO_DISTPERCENT));
...
int creaBodyAndShapeAutomat(INSTANCE *my) {
    ...
    if (((int *)LOCDWORD(mod_chipmunk, my, LOC_PARAMS)) != NULL) {
        ...
        GRAPH *map =
            bitmap_get(LOCDWORD(mod_chipmunk, my, LOC_FILE), LOCDWORD(mod_chipmunk, my, LOC_GRAPH));
        if (map == NULL)
            return 0;
        short int px;
        short int py;
