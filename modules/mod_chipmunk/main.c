...
char __pxtexport(mod_chipmunk, globals_def)[] =
    ...
    "float bias_coef=0.5;\n" /* Access and set with *(FLOAT *)GLOADDR(mod_chipmunk,GLO_BIAS_COEF))
                                */
    "float collision_slop=0.1;\n" /* Access and set with *(FLOAT
                                     *)GLOADDR(mod_chipmunk,GLO_COLLISION_SLOP)) */
...
static int modcpSpaceSegmentQueryFirst(INSTANCE *my, intptr_t *params) {
    ...
    cpSegmentQueryInfo info;
    cpShape *res =
        cpSpaceSegmentQueryFirst(modChipmunk_cpEspacio, *a, *b, params[2], params[3], &info);
...
static int modUpdateVelocity(INSTANCE *my, intptr_t *params) {
    cpBodyUpdateVelocity(LOCDWORD(mod_chipmunk, my, LOC_BODY),
                         ...
                         *(float *)GLOADDR(mod_chipmunk, GLO_DAMPING),
                         *(float *)GLOADDR(mod_chipmunk, GLO_INTERVAL) /
...
static int modUpdateVelocity2(INSTANCE *my, intptr_t *params) {
    cpBodyUpdateVelocity(LOCDWORD(mod_chipmunk, my, LOC_BODY),
                         cpv(*(float *)GLOADDR(mod_chipmunk, GLO_GRAVITY_X),
                             *(float *)GLOADDR(mod_chipmunk, GLO_GRAVITY_Y)),
                         *(float *)&params[0], *(float *)GLOADDR(mod_chipmunk, GLO_INTERVAL) /
                                                   (float)GLODWORD(mod_chipmunk, GLO_PHRESOLUTION));
    return 0;
}
...
static int modSleepWithGroup(INSTANCE *my, intptr_t *params) {
    ...
    if (!cpBodyIsSleeping(LOCDWORD(mod_chipmunk, b, LOC_BODY)))
        cpBodySleepWithGroup(LOCDWORD(mod_chipmunk, b, LOC_BODY),
                             LOCDWORD(mod_chipmunk, c, LOC_BODY));
    return 0;
}
...
static int modgetConstraints(INSTANCE *my, intptr_t *params) {
    ...
    //    if (bod)
    //    {
    //        if (indiceConstraints==NULL || indiceConstraints->sig== NULL || idConstraints!=
    //        params[0])
...
static int modSpaceSegmentQueryFirst(INSTANCE *my, intptr_t *params) {
    ...
    cpSegmentQueryInfo info;
    int id     = -1;
    cpShape *a = cpSpaceSegmentQueryFirst(
        modChipmunk_cpEspacio, cpv(*(float *)&params[0], *(float *)&params[1]),
        cpv(*(float *)&params[2], *(float *)&params[3]), lay, group, &info);
    if (a != NULL) {
...
static int modSegmentQueryHitPoint(INSTANCE *my, intptr_t *params) {
    ...
    info.shape = (cpShape *)inf->shape;
    cpVect resAux =
        cpSegmentQueryHitPoint(cpv(params[0], params[1]), cpv(params[2], params[3]), info);
    int *x = params[5];
    int *y = params[6];
    *x     = resAux.x;
    *y     = resAux.y;
...
static int modaddInanimateShape(INSTANCE *my, intptr_t *params) {
    ...
    if (params[0] == TYPE_BOX || params[0] == TYPE_CONVEX_POLYGON) {
        sha = cpPolyShapeNew(bod, params[3], params[4],
                             cpv(*(float *)&params[1], *(float *)&params[2]));
    } else {
        switch (params[0]) {
            case TYPE_CIRCLE:
...
//        cpSpaceAddPostStepCallback(modChipmunk_cpEspacio, (cpPostStepFunc)postStepRemove, NULL,
//        NULL);
//    }else{
//        modInitLista();
//        modAddLista(a);
...
int existe(int id) {
    ...
    if (id >= FIRST_INSTANCE_ID) {
        i = instance_get(id);
        if (i && (LOCDWORD(mod_chipmunk, i, LOC_RSTATUS) & ~STATUS_WAITING_MASK) >= STATUS_RUNNING)
            return 1;
        return 0;
    }
    ...
    while ((i = instance_get_by_type(id, &ctx))) {
        if ((LOCDWORD(mod_chipmunk, i, LOC_RSTATUS) & ~STATUS_WAITING_MASK) >= STATUS_RUNNING)
            return 1;
    }
...
void actualizaProcess(DataPointer dat) {
    ...
    float resolucion = LOCDWORD(mod_chipmunk, ins, LOC_RESOLUTION) > 0
                           ? LOCDWORD(mod_chipmunk, ins, LOC_RESOLUTION)
                           : 1.0f;

    if (LOCDWORD(mod_chipmunk, ins, LOC_X) != dat->x ||
        LOCDWORD(mod_chipmunk, ins, LOC_Y) != dat->y) {
    ...
    if (LOCDWORD(mod_chipmunk, ins, LOC_INCR_X) != 0 ||
        LOCDWORD(mod_chipmunk, ins, LOC_INCR_Y) != 0) {
        cuerpo->p.x =
            (LOCDWORD(mod_chipmunk, ins, LOC_INCR_X) + LOCDWORD(mod_chipmunk, ins, LOC_X)) /
            resolucion;
        cuerpo->p.y =
            (LOCDWORD(mod_chipmunk, ins, LOC_INCR_Y) + LOCDWORD(mod_chipmunk, ins, LOC_Y)) /
            resolucion;
        if (estatico)
            gEstatico = 1;
    ...
    if (gEstatico) {
        cpSpaceReindexShapesForBody(modChipmunk_cpEspacio,
                                    (cpBody *)LOCDWORD(mod_chipmunk, ins, LOC_BODY));
    }
}

void ActualizaLista(void *ptr,
                    void *data) // handler de colisiones, se sustituirá por los callbacks en breve
{
...
static int updatePhisc() // actualiza cada frame la física en general
{
    ...
    modChipmunk_cpEspacio->idleSpeedThreshold =
        *(float *)GLOADDR(mod_chipmunk, GLO_IDLESPEEDTHRESHOLD);
    modChipmunk_cpEspacio->sleepTimeThreshold =
        *(float *)GLOADDR(mod_chipmunk, GLO_SLEEPTIMETHRESHOLD);
    ...
    while (i) {
        if (LOCDWORD(mod_chipmunk, i, LOC_BODY) == 0 &&
            LOCDWORD(mod_chipmunk, i, LOC_SHAPETYPE) != 0) {
    ...
    for (auxIterator = modChipmunk_ListaCuerpos; auxIterator->sig != NULL;) {
        if (existe(auxIterator->sig->father)) {
            if (LOCDWORD(mod_chipmunk, instance_get(auxIterator->sig->father), LOC_SHAPETYPE) !=
                    auxIterator->sig->typeShape ||
                auxIterator->sig->estado == STATUS_DEAD ||
                (LOCDWORD(mod_chipmunk, instance_get(auxIterator->sig->father), LOC_STATIC) !=
                 cpBodyIsStatic(auxIterator->sig->body))) {
                if ((LOCDWORD(mod_chipmunk, instance_get(auxIterator->sig->father),
                              LOC_SHAPETYPE) != auxIterator->sig->typeShape &&
                     LOCDWORD(mod_chipmunk, instance_get(auxIterator->sig->father), LOC_BODY) ==
                         auxIterator->sig->body) ||
                    (LOCDWORD(mod_chipmunk, instance_get(auxIterator->sig->father), LOC_STATIC) !=
                     cpBodyIsStatic(auxIterator->sig->body))) {
...
ChipmunkFreeSpaceChildren()
{
    ...
    cpSpatialIndexEach(modChipmunk_cpEspacio->staticShapes,
(cpSpatialIndexIteratorFunc)&shapeFreeWrap, NULL);
    cpSpatialIndexEach(modChipmunk_cpEspacio->activeShapes,
(cpSpatialIndexIteratorFunc)&shapeFreeWrap, NULL);
...
DLSYSFUNCS __pxtexport(mod_chipmunk, exported_functions)[] = {
    ...
    {"DRAW_WATER", "IIIPIFFIII", TYPE_INT,
     modChipmunkPintaAgua}, // FILE,GRAPH,COLOR,LISTA IDS, TAM,tam particulas,umbral
