...
GRAPH *gr_read_png(const char *filename) {
    ...
    bitmap = bitmap_new(0, width, height, (color == PNG_COLOR_TYPE_GRAY)
                                              ? depth
                                              : (color == PNG_COLOR_TYPE_PALETTE)
                                                    ? 8
                                                    : (sys_pixel_format->depth == 16 ? 16 : 32));
    if (!bitmap) {
        png_destroy_read_struct(&png_ptr, &info_ptr, &end_info);
        free(rowpointers);
        free(row);
    ...
    if (color == PNG_COLOR_TYPE_GRAY) {
        for (n = 0; n < height; n++)
            rowpointers[n] = ((uint8_t *)bitmap->data) + n * bitmap->pitch;
        ...
        for (n = 0; n < height; n++) {
            ...
            for (x = 0; x < width; x++) {
                ...
                *ptr32 = *orig;
                ...
                /* DCelso */
                if ((color == PNG_COLOR_TYPE_RGB) && (png_get_bit_depth(png_ptr, info_ptr) == 24) &&
                    (png_get_valid(png_ptr, info_ptr, PNG_INFO_tRNS))) {
                    uint8_t *ptr8 = (uint8_t *)orig;
                    if ((ptr8[0] == trans_color->red) && (ptr8[1] == trans_color->green) &&
        ...
        for (n = 0; n < height; n++) {
            ...
            for (x = 0; x < width; x++) {
                ...
                if ((*orig) & 0x80000000) {
                    *ptr = ((*orig & Rmask) >> Rshift) | ((*orig & Gmask) >> Gshift) |
                           ((*orig & Bmask) >> Bshift);
                    if (!*ptr)
                        (*ptr)++;
                } else
...
int gr_save_png(GRAPH *gr, const char *filename) {
    ...
    if (gr->format->depth == 1) {
    ...
    } else if (gr->format->depth == 8) {
        /* 8 bits PNG file */
        png_set_IHDR(png_ptr, info_ptr, gr->width, gr->height, 8, PNG_COLOR_TYPE_PALETTE,
                     PNG_INTERLACE_NONE, PNG_COMPRESSION_TYPE_BASE, PNG_FILTER_TYPE_BASE);
        ...
        png_set_IHDR(png_ptr, info_ptr, gr->width, gr->height, 8, PNG_COLOR_TYPE_RGB_ALPHA,
                     PNG_INTERLACE_NONE, PNG_COMPRESSION_TYPE_BASE, PNG_FILTER_TYPE_BASE);
        ...
        if (gr->format->depth == 16) {
            for (k = 0; k < (unsigned)gr->height; k++) {
                ...
                for (i = 0; i < (unsigned)gr->width; i++) {
                    if (*orig == 0 && !(gr->info_flags & GI_NOCOLORKEY))
                        ...
                        *ptr = ((*orig & 0xf800) >> 8) | ((*orig & 0x07e0) << 5) |
                               ((*orig & 0x001f) << 19) | 0xFF000000;
                        /* Rearrange data */
                        ARRANGE_DWORD(ptr);
                    }
                    orig++;
            ...
            for (k = 0; k < (unsigned)gr->height; k++) {
                ...
                rowpointers[k] = (uint8_t *)ptr;
                for (i = 0; i < (unsigned)gr->width; i++) {
                    *ptr = ((*orig32 & 0xff000000)) | ((*orig32 & 0x00ff0000) >> 16) |
                           ((*orig32 & 0x0000ff00)) | ((*orig32 & 0x000000ff) << 16);
