...
int gr_save_fpg(int libid, const char *filename) {
    ...
    */
    ...
    for (bpp = n = 0; n < nmaps; n++) {
        if (lib->maps[n]) {
            if (bpp && lib->maps[n]->format->depth != bpp)
                return 0; /* save_fpg: don't allow save maps with differents bpp */
            bpp = lib->maps[n]->format->depth;
            if (!palette && bpp == 8 && lib->maps[n]->format->palette)
                palette = lib->maps[n]->format->palette->rgb;
        }
    }

    if (bpp == 32)
        strcpy((char *)header, F32_MAGIC);
    else if (bpp == 16)
        strcpy((char *)header, F16_MAGIC);
    else if (bpp == 8)
        strcpy((char *)header, FPG_MAGIC);
    else if (bpp == 1)
        strcpy((char *)header, F01_MAGIC);
    else
        return 0; /* No maps for save */
