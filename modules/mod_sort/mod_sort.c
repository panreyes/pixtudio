...
static int compare_string(const int *a, const int *b) {
    return strcmp(string_get(*(int *)((uint8_t *)a + keyoffset)),
                  string_get(*(int *)((uint8_t *)b + keyoffset)));
}
...
static int sort_variables(void *data, int key_offset, int key_type, int element_size,
                          int elements) {
...
int modsort_ksort(INSTANCE *my, int *params) {
    ...
    return sort_variables(data, (uint8_t *)key_data - (uint8_t *)data, copy.BaseType[0],
                          element_size, type->Count[0]);
}
...
static double GetData(uint8_t *Data, int pos, int *params) {
    ...
    if (params[4] == sizeof(uint16_t))
        return *(uint16_t *)(&Data[pos * params[1] + params[3]]);
    if (params[4] == sizeof(int) && params[5] == 0)
        return *(int *)(&Data[pos * params[1] + params[3]]);
    if (params[4] == sizeof(float) && params[5] == 1)
        return *(float *)(&Data[pos * params[1] + params[3]]);
    return 1;
}
...
static void QuickSort(uint8_t *Data, int inf, int sup, int *params) {
    ...
    do {
        while (GetData(Data, left, params) < middle && left < sup)
            left++;
        while (middle < GetData(Data, rigth, params) && rigth > inf)
            rigth--;
    ...
    if (inf < rigth)
        QuickSort(Data, inf, rigth, params);
    if (left < sup)
        QuickSort(Data, left, sup, params);
}
