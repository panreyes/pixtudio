...
enum { COLLISION_NORMAL = 0, COLLISION_BOX, COLLISION_CIRCLE };
...
int grproc_get_angle(INSTANCE *my, int *params) {
    ...
    if (my && b) {
        double dx =
            (double)((int)(LOCINT32(mod_grproc, b, COORDX) - LOCINT32(mod_grproc, my, COORDX)));
        double dy =
            (double)((int)(LOCINT32(mod_grproc, b, COORDY) - LOCINT32(mod_grproc, my, COORDY)));
        int angle;
...
static int get_distance_proc(INSTANCE *a, INSTANCE *b) {
    if (a && b)
        return (get_distance_xy(a, LOCINT32(mod_grproc, b, COORDX), LOCINT32(mod_grproc, b, COORDY),
                                LOCINT32(mod_grproc, b, RESOLUTION)));
    return -1;
}
...
int grproc_get_real_point(INSTANCE *my, int *params) {
    ...
    } else if ((uint32_t)params[0] >= b->ncpoints || params[0] < 0)
        return 0;
    ...
    if (LOCINT32(mod_grproc, my, GRAPHSIZEX) == 100 &&
        LOCINT32(mod_grproc, my, GRAPHSIZEY) == 100) {
...
static void draw_at(GRAPH *dest, int x, int y, REGION *r, INSTANCE *i) {
    ...
    if (scalex == 100 && scaley == 100)
        scalex = scaley = LOCINT32(mod_grproc, i, GRAPHSIZE);
...
static int check_collision_with_mouse(INSTANCE *proc1, int colltype) {
    ...
    if (colltype == COLLISION_NORMAL) {
        ...
        if (bmp && bmp->format->depth != sys_pixel_format->depth) {
            bitmap_destroy(bmp);
            bmp = NULL;
        }
        if (!bmp)
            bmp = bitmap_new(0, 2, 1, sys_pixel_format->depth);
        if (!bmp)
            return 0;
...
static int check_collision_circle(INSTANCE *proc1, GRAPH *bmp1, REGION *bbox1, INSTANCE *proc2) {
    ...
    bmp2 = instance_collision_graph(proc2);
    if (!bmp2)
        return 0;
    ...
    if (get_distance(cx1, cy1, 0, cx2, cy2, 0) < ((dx1 + dy1) / 2 + (dx2 + dy2) / 2) / 2)
        return 1;
...
static int __collision(INSTANCE *my, int id, int colltype) {
    ...
    if (id >= FIRST_INSTANCE_ID)
        return (((ptr = instance_get(id)) && ctype == LOCDWORD(mod_grproc, ptr, CTYPE))
    ...
    if (!id) {
        ...
        while (ptr) {
            if (ptr != my && ctype == LOCDWORD(mod_grproc, ptr, CTYPE) &&
                ((status = (LOCDWORD(mod_grproc, ptr, STATUS) & ~STATUS_WAITING_MASK)) ==
                     STATUS_RUNNING ||
