...
typedef struct { float x, y; } _POINTF;
...
static int compare_vertex_y(const void *pa, const void *pb) {
...
/*
static void draw_span_1to1(GRAPH * dest, GRAPH * orig, int x, int y, int pixels, int s, int t, int
incs, int inct)
{
...
static void draw_span_1to1_nocolorkey(GRAPH * dest, GRAPH * orig, int x, int y, int pixels, int s,
int t, int incs, int inct)
{
    uint8_t *ptr = (uint8_t *)dest->data + dest->pitch * y + x/8 ;
    int cs = s, ct = t, i;
...
static void draw_span_1to8(GRAPH *dest, GRAPH *orig, int x, int y, int pixels, int s, int t,
                           int incs, int inct) {
...
static void draw_span_8to8_nocolorkey(GRAPH *dest, GRAPH *orig, int x, int y, int pixels, int s,
                                      int t, int incs, int inct) {
    uint8_t *ptr = (uint8_t *)dest->data + dest->pitch * y + x;
    int cs = s, ct = t;
...
static void draw_span_8to8(GRAPH *dest, GRAPH *orig, int x, int y, int pixels, int s, int t,
                           int incs, int inct) {
    uint8_t *ptr = (uint8_t *)dest->data + dest->pitch * y + x;
    int cs = s, ct = t;
...
static void draw_span_8to8_ablend(GRAPH *dest, GRAPH *orig, int x, int y, int pixels, int s, int t,
                                  int incs, int inct) {
    uint8_t *ptr = (uint8_t *)dest->data + dest->pitch * y + x;
    int cs = s, ct = t;
    ...
    while (pixels--) {
        ...
        if (*tex)
            *ptr = (uint8_t)blend_func(*tex, *ptr);
        ptr++;
        cs += incs, ct += inct;
    }
}
...
static void draw_span_1to16(GRAPH *dest, GRAPH *orig, int x, int y, int pixels, int s, int t,
                            int incs, int inct) {
...
static void draw_span_8to16(GRAPH *dest, GRAPH *orig, int x, int y, int pixels, int s, int t,
                            int incs, int inct) {
    uint16_t *ptr = (uint16_t *)dest->data + (dest->pitch * y >> 1) + x;
    int cs = s, ct = t;
...
static void draw_span_8to16_ablend(GRAPH *dest, GRAPH *orig, int x, int y, int pixels, int s, int t,
                                   int incs, int inct) {
    uint16_t *ptr = (uint16_t *)dest->data + (dest->pitch * y >> 1) + x;
    int cs = s, ct = t;
    ...
    while (pixels--) {
        uint8_t *tex = (uint8_t *)orig->data + orig->pitch * (ct >> 16) + (cs >> 16);
        if (*tex)
            *ptr = blend_func(pcolorequiv[*tex], *ptr);
        ptr++;
        cs += incs, ct += inct;
    }
}

static void draw_span_8to16_nocolorkey(GRAPH *dest, GRAPH *orig, int x, int y, int pixels, int s,
                                       int t, int incs, int inct) {
    uint16_t *ptr = (uint16_t *)dest->data + (dest->pitch * y >> 1) + x;
    int cs = s, ct = t;
...
static void draw_span_8to32(GRAPH *dest, GRAPH *orig, int x, int y, int pixels, int s, int t,
                            int incs, int inct) {
...
static void draw_span_8to32_ablend(GRAPH *dest, GRAPH *orig, int x, int y, int pixels, int s, int t,
                                   int incs, int inct) {
    uint32_t *ptr = (uint32_t *)dest->data + (dest->pitch * y >> 2) + x;
    int cs = s, ct = t;
    ...
    while (pixels--) {
        uint8_t *tex = (uint8_t *)orig->data + orig->pitch * (ct >> 16) + (cs >> 16);
        if (*tex)
            *ptr = (*ptr & 0xff000000) | blend_func(pcolorequiv[*tex], *ptr);
        ptr++;
        cs += incs, ct += inct;
    }
}

static void draw_span_8to32_nocolorkey(GRAPH *dest, GRAPH *orig, int x, int y, int pixels, int s,
                                       int t, int incs, int inct) {
    uint32_t *ptr = (uint32_t *)dest->data + (dest->pitch * y >> 2) + x;
    int cs = s, ct = t;
...
static void draw_span_16to16(GRAPH *dest, GRAPH *orig, int x, int y, int pixels, int s, int t,
                             int incs, int inct) {
    uint16_t *ptr = (uint16_t *)dest->data + (dest->pitch * y >> 1) + x;
    int cs = s, ct = t;
    uint16_t *tex;
...
static void draw_span_16to16_ablend(GRAPH *dest, GRAPH *orig, int x, int y, int pixels, int s,
                                    int t, int incs, int inct) {
    uint16_t *ptr = (uint16_t *)dest->data + (dest->pitch * y >> 1) + x;
    int cs = s, ct = t;
    uint16_t *tex;
...
static void draw_span_16to16_nocolorkey(GRAPH *dest, GRAPH *orig, int x, int y, int pixels, int s,
                                        int t, int incs, int inct) {
    uint16_t *ptr = (uint16_t *)dest->data + (dest->pitch * y >> 1) + x;
    int cs = s, ct = t;
    uint16_t *tex;
...
static void draw_span_16to32(GRAPH *dest, GRAPH *orig, int x, int y, int pixels, int s, int t,
                             int incs, int inct) {
    uint32_t *ptr = (uint32_t *)dest->data + (dest->pitch * y >> 2) + x;
    int cs = s, ct = t;
    uint16_t *tex;
    ...
    while (pixels--) {
        tex = (uint16_t *)orig->data + (orig->pitch * (ct >> 16) >> 1) + (cs >> 16);
        if (*tex)
            *ptr = (*ptr & 0xff000000) | ((*tex & 0xf800) << 8) | ((*tex & 0x07e0) << 5) |
                   ((*tex & 0x001f) << 3);
        ptr++;
        cs += incs, ct += inct;
    }
...
static void draw_span_16to32_ablend(GRAPH *dest, GRAPH *orig, int x, int y, int pixels, int s,
                                    int t, int incs, int inct) {
    uint32_t *ptr = (uint32_t *)dest->data + (dest->pitch * y >> 2) + x;
    int cs = s, ct = t;
    uint16_t *tex;
    ...
    while (pixels--) {
        ...
        if (*tex)
            *ptr =
                (*ptr & 0xff000000) |
                blend_func(((*tex & 0xf800) << 8) | ((*tex & 0x07e0) << 5) | ((*tex & 0x001f) << 3),
                           *ptr);
        ptr++;
        cs += incs, ct += inct;
    }
...
static void draw_span_16to32_nocolorkey(GRAPH *dest, GRAPH *orig, int x, int y, int pixels, int s,
                                        int t, int incs, int inct) {
    uint32_t *ptr = (uint32_t *)dest->data + (dest->pitch * y >> 2) + x;
    int cs = s, ct = t;
    uint16_t *tex;
...
static void draw_span_1to32(GRAPH *dest, GRAPH *orig, int x, int y, int pixels, int s, int t,
                            int incs, int inct) {
    uint32_t *ptr = (uint32_t *)dest->data + (dest->pitch * y >> 2) + x;
    int cs = s, ct = t;
    ...
    while (pixels--) {
        ...
        uint8_t mask = (0x80 >> ((cs >> 16) & 7));
        if (*tex & mask)
            *ptr = (*ptr & 0xff000000) | pixel_color32;
        ptr++;
        cs += incs, ct += inct;
    }
}
...
static void draw_span_32to32(GRAPH *dest, GRAPH *orig, int x, int y, int pixels, int s, int t,
                             int incs, int inct) {
    uint32_t *ptr = (uint32_t *)dest->data + (dest->pitch * y >> 2) + x;
    int cs = s, ct = t;
    uint32_t *tex;
...
static void draw_span_32to32_ablend(GRAPH *dest, GRAPH *orig, int x, int y, int pixels, int s,
                                    int t, int incs, int inct) {
    uint32_t *ptr = (uint32_t *)dest->data + (dest->pitch * y >> 2) + x;
    int cs = s, ct = t;
    uint32_t *tex;
...
static void draw_span_32to32_nocolorkey(GRAPH *dest, GRAPH *orig, int x, int y, int pixels, int s,
                                        int t, int incs, int inct) {
    uint32_t *ptr = (uint32_t *)dest->data + (dest->pitch * y >> 2) + x;
    int cs = s, ct = t;
    uint32_t *tex;
...
static void draw_hspan_1to8(uint8_t *scr, uint8_t *tex, int pixels, int incs, int l, int scr_inc,
                            int tex_inc) {
...
static void draw_hspan_8to8_nocolorkey(uint8_t *scr, uint8_t *tex, int pixels, int incs, int l,
                                       int scr_inc, int tex_inc) {
    int i;
    uint8_t *_scr = (uint8_t *)scr, *_tex = (uint8_t *)tex;
...
static void draw_hspan_8to8_ablend(uint8_t *scr, uint8_t *tex, int pixels, int incs, int l,
                                   int scr_inc, int tex_inc) {
    int i;
    uint8_t *_scr = (uint8_t *)scr, *_tex = (uint8_t *)tex;
    ...
    while (l--) {
        for (i = pixels; i--;) {
            if (*tex)
                *scr = (uint8_t)blend_func(*tex, *scr);
            scr++;
            tex += incs;
        }
        scr = _scr += scr_inc;
...
static void draw_hspan_8to8(uint8_t *scr, uint8_t *tex, int pixels, int incs, int l, int scr_inc,
                            int tex_inc) {
    int i;
    uint8_t *_scr = (uint8_t *)scr, *_tex = (uint8_t *)tex;
...
static void draw_hspan_1to16(uint16_t *scr, uint8_t *tex, int pixels, int incs, int l, int scr_inc,
                             int tex_inc) {
    int mask, omask = (0x80 >> (posx & 7));
    int i;
    uint8_t *_scr = (uint8_t *)scr, *_tex = (uint8_t *)tex;
...
static void draw_hspan_8to16(uint16_t *scr, uint8_t *tex, int pixels, int incs, int l, int scr_inc,
                             int tex_inc) {
    int i;
    uint8_t *_scr = (uint8_t *)scr, *_tex = (uint8_t *)tex;
...
static void draw_hspan_8to16_ablend(uint16_t *scr, uint8_t *tex, int pixels, int incs, int l,
                                    int scr_inc, int tex_inc) {
    int i;
    uint8_t *_scr = (uint8_t *)scr, *_tex = (uint8_t *)tex;
    ...
    while (l--) {
        for (i = pixels; i--;) {
            if (*tex)
                *scr = blend_func(pcolorequiv[*tex], *scr);
            scr++;
            tex += incs;
        }
        scr = (uint16_t *)(_scr += scr_inc);
...
static void draw_hspan_8to16_nocolorkey(uint16_t *scr, uint8_t *tex, int pixels, int incs, int l,
                                        int scr_inc, int tex_inc) {
    int i;
    uint8_t *_scr = (uint8_t *)scr, *_tex = (uint8_t *)tex;
...
static void draw_hspan_16to16(uint16_t *scr, uint16_t *tex, int pixels, int incs, int l,
                              int scr_inc, int tex_inc) {
    int i;
    uint8_t *_scr = (uint8_t *)scr, *_tex = (uint8_t *)tex;
    ...
    while (l--) {
        ...
        scr = (uint16_t *)(_scr += scr_inc);
        tex = (uint16_t *)(_tex += tex_inc);
    }
}

static void draw_hspan_16to16_ablend(uint16_t *scr, uint16_t *tex, int pixels, int incs, int l,
                                     int scr_inc, int tex_inc) {
    int i;
    uint8_t *_scr = (uint8_t *)scr, *_tex = (uint8_t *)tex;
...
static void draw_hspan_16to16_nocolorkey(uint16_t *scr, uint16_t *tex, int pixels, int incs, int l,
                                         int scr_inc, int tex_inc) {
    int i;
    uint8_t *_scr = (uint8_t *)scr, *_tex = (uint8_t *)tex;
...
static void draw_hspan_1to32(uint32_t *scr, uint8_t *tex, int pixels, int incs, int l, int scr_inc,
                             int tex_inc) {
    int mask, omask = (0x80 >> (posx & 7));
    int i;
    uint8_t *_scr = (uint8_t *)scr, *_tex = (uint8_t *)tex;
    ...
    while (l--) {
        ...
        for (i = pixels; i--;) {
            if (*tex & mask)
                *scr = (*scr & 0xff000000) | pixel_color32;
            scr++;
            if (incs < 0) {
                if (mask == 0x80)
                    mask = 0x01, tex--;
...
static void draw_hspan_8to32(uint32_t *scr, uint8_t *tex, int pixels, int incs, int l, int scr_inc,
                             int tex_inc) {
    int i;
    uint8_t *_scr = (uint8_t *)scr, *_tex = (uint8_t *)tex;
...
static void draw_hspan_8to32_ablend(uint32_t *scr, uint8_t *tex, int pixels, int incs, int l,
                                    int scr_inc, int tex_inc) {
    int i;
    uint8_t *_scr = (uint8_t *)scr, *_tex = (uint8_t *)tex;
    ...
    while (l--) {
        for (i = pixels; i--;) {
            if (*tex)
                *scr = (*scr & 0xff000000) | blend_func(pcolorequiv[*tex], *scr);
            scr++;
            tex += incs;
        }
        scr = (uint32_t *)(_scr += scr_inc);
...
static void draw_hspan_8to32_nocolorkey(uint32_t *scr, uint8_t *tex, int pixels, int incs, int l,
                                        int scr_inc, int tex_inc) {
    int i;
    uint8_t *_scr = (uint8_t *)scr, *_tex = (uint8_t *)tex;
...
static void draw_hspan_16to32(uint32_t *scr, uint16_t *tex, int pixels, int incs, int l,
                              int scr_inc, int tex_inc) {
    int i;
    uint8_t *_scr = (uint8_t *)scr, *_tex = (uint8_t *)tex;
    ...
    while (l--) {
        for (i = pixels; i--;) {
            if (*tex)
                *scr = (*scr & 0xff000000) | ((*tex & 0xf800) << 8) | ((*tex & 0x07e0) << 5) |
                       ((*tex & 0x001f) << 3);
            scr++;
            tex += incs;
        }
...
static void draw_hspan_16to32_ablend(uint32_t *scr, uint16_t *tex, int pixels, int incs, int l,
                                     int scr_inc, int tex_inc) {
    int i;
    uint8_t *_scr = (uint8_t *)scr, *_tex = (uint8_t *)tex;
    ...
    while (l--) {
        for (i = pixels; i--;) {
            if (*tex)
                *scr = (*scr & 0xff000000) |
                       blend_func(((*tex & 0xf800) << 8) | ((*tex & 0x07e0) << 5) |
                                      ((*tex & 0x001f) << 3),
                                  *scr);
            scr++;
            tex += incs;
...
static void draw_hspan_16to32_nocolorkey(uint32_t *scr, uint16_t *tex, int pixels, int incs, int l,
                                         int scr_inc, int tex_inc) {
    int i;
    uint8_t *_scr = (uint8_t *)scr, *_tex = (uint8_t *)tex;
...
static void draw_hspan_32to32(uint32_t *scr, uint32_t *tex, int pixels, int incs, int l,
                              int scr_inc, int tex_inc) {
    int i;
    uint8_t *_scr = (uint8_t *)scr, *_tex = (uint8_t *)tex;
    uint32_t r, g, b, c;
    ...
    while (l--) {
        ...
        scr = (uint32_t *)(_scr += scr_inc);
        tex = (uint32_t *)(_tex += tex_inc);
    }
}

static void draw_hspan_32to32_ablend(uint32_t *scr, uint32_t *tex, int pixels, int incs, int l,
                                     int scr_inc, int tex_inc) {
    int i;
    uint8_t *_scr = (uint8_t *)scr, *_tex = (uint8_t *)tex;
    uint32_t r, g, b, c;
...
static void draw_hspan_32to32_nocolorkey(uint32_t *scr, uint32_t *tex, int pixels, int incs, int l,
                                         int scr_inc, int tex_inc) {
    int i;
    uint8_t *_scr = (uint8_t *)scr, *_tex = (uint8_t *)tex;
...
void gr_get_bbox(REGION *dest, REGION *clip, int x, int y, int flags, int angle, int scalex,
    ...
    for (i = 1; i < 4; i++) {
        if (min.x > corners[i].x)
            min.x = corners[i].x;
        if (max.x < corners[i].x)
            max.x = corners[i].x;
        if (min.y > corners[i].y)
            min.y = corners[i].y;
        if (max.y < corners[i].y)
            max.y = corners[i].y;
    }
...
void gr_rotated_blit(GRAPH *dest, REGION *clip, int scrx, int scry, int flags, int angle,
    ...
    if (scrbitmap && dest == scrbitmap) {
        ...
        {
            if (gr->format->depth == 8) {
                pcolorequiv = gr->format->palette ? gr->format->palette->colorequiv
                                                  : sys_pixel_format->palette
                                                        ? sys_pixel_format->palette->colorequiv
                                                        : default_colorequiv;
        ...
        for (i = vertex[0].y / 1000; i < yf; i++) {
            ...
            if (i >= min.y && i <= max.y) {
                ...
                if (x2 > x) {
                    draw_span(dest, gr, x, i, x2 - x + 1, (int)(s * 65536), (int)(t * 65536),
                              (int)((s2 - s) / (x2 - x) * 65536),
                              (int)((t2 - t) / (x2 - x) * 65536));
                }
            }
            ...
            if (right_pos == right_steps) {
                ...
                right_steps = (right_end->y - right_start->y) / 1000;
                if (right_steps > 0) {
                    right_texture_increment.x =
                        (right_end->s - right_start->s) / (float)(right_steps);
                    right_texture_increment.y =
                        (right_end->t - right_start->t) / (float)(right_steps);
                }
            }
        }
...
void gr_blit(GRAPH *dest, REGION *clip, int scrx, int scry, int flags, uint8_t modr, uint8_t modg,
    ...
    if (scrbitmap && dest == scrbitmap) {
        ...
        if (clip) {
            min.x = MAX(clip->x, 0);
            min.y = MAX(clip->y, 0);
            max.x = MIN(clip->x2, dest->width);
            max.y = MIN(clip->y2, dest->height);
        } else {
            min.x = 0;
            min.y = 0;
            max.x = dest->width - 1;
        ...
        if (p > 0)
            draw_hspan(scr, tex, p, direction, l, scr_inc, tex_inc);
