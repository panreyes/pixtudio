...
int gr_map_rgb(PIXEL_FORMAT *format, int r, int g, int b) {
    if (format->depth == 32) {
        ...
        return ((r >> format->Rloss) << format->Rshift) | ((g >> format->Gloss) << format->Gshift) |
...
int gr_map_rgba(PIXEL_FORMAT *format, int r, int g, int b, int a) {
    if (format->depth == 32) {
        return ((a << 24) & 0xff000000) | ((r << 16) & 0x00ff0000) | ((g << 8) & 0x0000ff00) |
               ((b)&0x000000ff);
    } else if (format->depth == 16) {
        return ((r >> format->Rloss) << format->Rshift) | ((g >> format->Gloss) << format->Gshift) |
               ((b >> format->Bloss) << format->Bshift);
...
void pal_refresh(PALETTE *pal) {
    if (sys_pixel_format->depth > 8) {
        ...
        if (!pal) {
            ...
            while (pal) {
                for (n = 256; n--;)
                    pal->colorequiv[n] =
                        gr_map_rgb(sys_pixel_format, pal->rgb[n].r, pal->rgb[n].g, pal->rgb[n].b);
                pal = pal->next;
            }
        } else {
            for (n = 256; n--;)
    ...
    } else if (!pal || pal == sys_pixel_format->palette)
        palette_changed = 1;
}
...
void pal_destroy(PALETTE *pal) {
    ...
    if (pal == first_palette)
        first_palette = pal->next;
...
int pal_map_assign(int libid, int mapcode, PALETTE *palid) {
    ...
    if (!bmp /*|| !palid*/ || bmp->format->depth != 8)
        return 0;
...
int pal_map_remove(int libid, int mapcode) {
    ...
    if (!bmp || bmp->format->depth != 8)
        return 0;

    if (bmp->format->palette)
        pal_destroy(bmp->format->palette);
...
void gr_roll_palette(int color0, int num, int inc) {
    ...
    {
        inc = -inc;
        memmove(&sys_pixel_format->palette->rgb[color0 + inc],
                &sys_pixel_format->palette->rgb[color0], sizeof(rgb_component) * (num - inc));
        memcpy(&sys_pixel_format->palette->rgb[color0], &backup[color0 + num - inc],
               sizeof(rgb_component) * inc);
    } else /* Izquierda */
    {
        memmove(&sys_pixel_format->palette->rgb[color0],
                &sys_pixel_format->palette->rgb[color0 + inc], sizeof(rgb_component) * (num - inc));
        memcpy(&sys_pixel_format->palette->rgb[color0 + num - inc], &backup[color0],
               sizeof(rgb_component) * inc);
    }
...
void gr_get_rgb(int color, int *r, int *g, int *b) {
    ...
    (*r) = ((color & sys_pixel_format->Rmask) >> sys_pixel_format->Rshift)
           << sys_pixel_format->Rloss;
    (*g) = ((color & sys_pixel_format->Gmask) >> sys_pixel_format->Gshift)
           << sys_pixel_format->Gloss;
    (*b) = ((color & sys_pixel_format->Bmask) >> sys_pixel_format->Bshift)
           << sys_pixel_format->Bloss;
}
...
void gr_get_rgba(int color, int *r, int *g, int *b, int *a) {
    ...
    if (sys_pixel_format->depth == 32) {
        (*a) = ((color & sys_pixel_format->Amask) >> sys_pixel_format->Ashift)
               << sys_pixel_format->Aloss;
    }
}
...
int gr_rgb_depth(int depth, int r, int g, int b) {
    ...
    color = ((r >> pf->Rloss) << pf->Rshift) | ((g >> pf->Gloss) << pf->Gshift) |
...
void gr_make_trans_table() {
    ...
    for (a = 0; a < 256; a++) {
        ...
        for (b = 0; b < a; b++)
            trans_table[a][b] = trans_table[b][a] = find_nearest_color(
                NULL, b, a, r1 + rgb[b].r / 2, g1 + rgb[b].g / 2, b1 + rgb[b].b / 2);
